...
def test_ttft_lower_with_cache_hits():
    """
    Requests with cache hits should have lower TTFT because fewer tokens need prefill.
    Run twice with same config: second run should see some hits and potentially lower TTFT.
    We compare cold run (unique data) vs warm run (repeated data) on a single instance.
    """
    import kvcm_py_optimizer as kvcm_mod
    from schedule_simulator.schedule_emulator.hierarchical_cache_adapter import HierarchicalCacheAdapter
    ...
    platform = PlatformConfig(device="H20", disk_read_bandwidth_gb=2.0, memory_read_bandwidth_gb=16.0)

    # Cold run
    random.seed(100)
    numpy.random.seed(100)
    cold_cfg = BenchmarkConfig(
        num_prompts=20, min_input_length=200, max_input_length=500,
        min_output_length=50, max_output_length=100, disable_tqdm=True,
    )
    sched_cfg = SchedulerConfig("Qwen2.5-3B", hicache_storage_backend="hf3fs",
                                 hicache_storage_prefetch_policy="best_effort")
    cold_runner = BenchmarkRunner(
        benchmark_config=cold_cfg, scheduler_config=sched_cfg, platform_config=platform,
    )
    cold_adapter = HierarchicalCacheAdapter(
        manager=mgr, engine_instance_id="P0", platform_config=platform,
        kv_cache_space_per_token=cold_runner.scheduler_emulator.kv_cache_space_per_token,
        page_size=cold_runner.scheduler_emulator.scheduler_config.page_size,
        global_values=cold_runner.scheduler_emulator.global_values,
        prefetch_stop_policy="best_effort",
    )
    cold_runner.scheduler_emulator.tree_cache = cold_adapter
    cold_runner.scheduler_emulator.policy = SchedulePolicy(
        sched_cfg.schedule_policy, cold_adapter, cold_runner.scheduler_emulator.time_predictor
    )
    cold_metrics = cold_runner.run_benchmark_emulation()
    cold_ttft = cold_metrics["mean_ttft_ms"]
    cold_hit = cold_adapter.total_engine_hit_blocks

    # Warm run - same data, cache should be populated from cold run
    random.seed(100)
    numpy.random.seed(100)
    warm_runner = BenchmarkRunner(
        benchmark_config=cold_cfg, scheduler_config=sched_cfg, platform_config=platform,
    )
    warm_adapter = HierarchicalCacheAdapter(
        manager=mgr, engine_instance_id="P0", platform_config=platform,
        kv_cache_space_per_token=warm_runner.scheduler_emulator.kv_cache_space_per_token,
        page_size=warm_runner.scheduler_emulator.scheduler_config.page_size,
        global_values=warm_runner.scheduler_emulator.global_values,
        prefetch_stop_policy="best_effort",
    )
    warm_runner.scheduler_emulator.tree_cache = warm_adapter
    warm_runner.scheduler_emulator.policy = SchedulePolicy(
        sched_cfg.schedule_policy, warm_adapter, warm_runner.scheduler_emulator.time_predictor
    )
    warm_metrics = warm_runner.run_benchmark_emulation()
    warm_ttft = warm_metrics["mean_ttft_ms"]
    warm_hit = warm_adapter.total_engine_hit_blocks

    assert warm_hit > cold_hit, (
        f"Warm run should have more hits: cold={cold_hit}, warm={warm_hit}"
    )
    assert warm_ttft < cold_ttft * 1.1, (
        f"Warm TTFT ({warm_ttft:.1f}ms) should be <= cold TTFT ({cold_ttft:.1f}ms)"
    )
    print(f"[ttft_impact] Cold: TTFT={cold_ttft:.1f}ms hits={cold_hit}, "
          f"Warm: TTFT={warm_ttft:.1f}ms hits={warm_hit}")
