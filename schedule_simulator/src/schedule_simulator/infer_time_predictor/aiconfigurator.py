...
# from deepestim.llmperf.analyzers.base import BaseExecutionAnalyzer
# from deepestim.llmperf.types import ForwardInfo, PerfConfig, PerfMode, RequestInfo
...
# Map the kunlun common's data type to ai configurator data type.
...
def _import_xgboost():
    ...
    Note: this repo vendors the xgboost source tree at `<repo_root>/xgboost/`, which can shadow
    the real python package depending on cwd/sys.path. We try:
    - normal import
    - repo-local python-package import (adds `<repo_root>/xgboost/python-package` to sys.path)
    """
...
def _load_bucket_models(model_dir: str):
    ...
    for fn in os.listdir(model_dir):
        ...
        print(f"✅ Loaded bucket XGBoost model bs{lo}_{hi}: {p}")
...
def _build_xgb_feature_maxbs_2(
    reqs: list[ScheduleRequest], *, max_bs: int
) -> np.ndarray:
    """
...
def _xgb_predict_ratio(reqs: list[ScheduleRequest], xgb_reg, *, max_bs: int) -> float:
    if len(reqs) > max_bs:
        raise ValueError(
...
class AIConfiguratorTimePredictor(InferTimePredictor):
    def __init__(
        ...
        if database is None:
            raise ValueError("Fail to initialize the database.")
    ...
    def get_perf_model(self) -> models.BaseModel:
        model_config = config.ModelConfig(
            pp_size=self.config.pp_size,
            tp_size=self.config.tp_size,
            moe_tp_size=self.config.tp_size,  # FIXME
            moe_ep_size=self.config.ep_size,
            attention_dp_size=self.config.dp_size,  # FIXME
            gemm_quant_mode=MAP_DTYPE_TO_GEMMQuantMode.get(
                self.config.data_type, GEMMQuantMode.float16
            ),
            moe_quant_mode=MAP_DTYPE_TO_MoEQuantMode.get(
                self.config.data_type, MoEQuantMode.float16
            ),
            kvcache_quant_mode=MAP_DTYPE_TO_KVCacheQuantMode.get(
                self.config.kv_cache_data_type, KVCacheQuantMode.float16
            ),
            fmha_quant_mode=MAP_DTYPE_TO_FMHAQuantMode.get(
                self.config.kv_cache_data_type, FMHAQuantMode.float16
            ),
            comm_quant_mode=MAP_DTYPE_TO_CommQunatMode.get(
                self.config.data_type, CommQuantMode.half
            ),
            workload_distribution="power_law_1.2",
        )

        if self.model.model_type in ["qwen", "qwen2", "qwen3", "llama", "chatglm"]:
        ...
        elif self.model.model_type in ["deepseek_v3", "kimi_k2"]:
            SupportedModels.update(
                {
                    self.model.name: [
                        "DEEPSEEK",
        ...
        elif self.model.model_type in ["qwen3_moe"]:
            SupportedModels.update(
                {
                    self.model.name: [
                        "MOE",
    ...
    def predict_infer_time(self, batch: ScheduleBatch) -> float:
        infer_time = 0
        if batch.is_decode():
            # Decode: osl=2, isl = mean(past_kv_length)
            ...
            if len(self.xgb_bucket_models) > 0:
                ...
                if picked is not None:
                    ...
                    # If XGBoost is enabled, it predicts ratio: aic_gen_attn_ms / measured_gen_attn_ms.
                    # To make AIC's gen_attn closer to measured, scale it by (1 / pred_ratio).
                    pred_ratio = _xgb_predict_ratio(
                        batch.reqs, xgb_reg, max_bs=xgb_max_bs
                    )
                    gen_attn_scale = 1.0 / pred_ratio
                    print(
                        f"Using XGBoost model for batch_size={batch.batch_size}, predicted ratio={pred_ratio:.4f}"
                    )
            if gen_attn_scale < 0:
            ...
            # Prefill: osl=1, isl = mean(past_kv + input), prefix = mean(past_kv)
        ...
        if summary.check_oom():
            print("OOM detected during estimation.")
