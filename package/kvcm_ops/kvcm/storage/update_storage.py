...
def create_update_storage_data(
        args,
        storage_type: str,
        storage_spec: dict,
        pace_storage_type=None,
    ...
    integrity = gen_integrity_config_data(args, existing_integrity)
...
def get_existing_storage(host: str, trace_id: str, unique_name: str, verbose: bool):
    ...
    if status_code not in ("OK", 1):
        raise RuntimeError(
            f"list storage failed while resolving existing storage config, status: {status_code}")

    for storage in result.get("storage", []):
        if storage.get("global_unique_name") == unique_name:
            return storage
    raise RuntimeError(f"storage '{unique_name}' does not exist")


def get_existing_pace_identity(storage: dict, unique_name: str):
...
def handle_nfs(args):
    existing = get_existing_storage(args.host, args.trace_id, args.unique_name, args.verbose)
    storage_spec = gen_nfs_config_data(args)
    data = create_update_storage_data(args, "nfs", storage_spec, existing_integrity=existing.get("integrity"))
    http_post_and_print(args.host, data, args.verbose)


def handle_pace(args):
    existing = get_existing_storage(args.host, args.trace_id, args.unique_name, args.verbose)
    media_type, pace_storage_type = get_existing_pace_identity(existing, args.unique_name)
    ...
    data = create_update_storage_data(
        ...
        "tair_mem_pool",
        storage_spec,
        pace_storage_type=pace_storage_type,
        existing_integrity=existing.get("integrity"))
    http_post_and_print(args.host, data, args.verbose)
...
def handle_3fs(args):
    ...
    data = create_update_storage_data(args, "threefs", storage_spec, existing_integrity=existing.get("integrity"))
    http_post_and_print(args.host, data, args.verbose)
...
def handle_event_report(args):
    ...
    data = create_update_storage_data(
        args, "event_report", storage_spec, existing_integrity=existing.get("integrity"))
    http_post_and_print(args.host, data, args.verbose)
