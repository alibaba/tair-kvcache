...
LruCacheIndexer::LruCacheIndexer(int64_t max_key_count) : max_key_count_(max_key_count) {}
...
void LruCacheIndexer::Init(const std::vector<double> &capacity_gb,
    ...
    if (linear_step_ == 0) {
        avg_bytes_per_block = size_full_only_;
    } else {
        avg_bytes_per_block = ((linear_step_ - 1) * size_full_only_ + size_full_linear_) / linear_step_;
    }
    ...
    if (max_cache_bytes > 0) {
        max_cache_ = NewLRUCache(static_cast<size_t>(max_cache_bytes),
                                 0,
                                 false,
                                 false,
                                 0.0,
...
void LruCacheIndexer::ProcessKeysLinearAttention(const std::vector<int64_t> &keys,
    ...
    // Effective reuse = last checkpoint within prefix + 1, or 0 if none.
    for (size_t j = 0; j < num_caps; j++) {
