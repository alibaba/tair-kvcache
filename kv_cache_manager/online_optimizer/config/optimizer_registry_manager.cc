...
OptimizerRegistryManager::OptimizerRegistryManager(const std::string &registry_uri)
    : registry_uri_(registry_uri) {}
...
bool OptimizerRegistryManager::Init() {
    ...
    if (!storage_) {
        KVCM_LOG_ERROR("OptimizerRegistryManager: failed to create storage backend for uri[%s]",
                       registry_uri_.c_str());
        return false;
    }
...
ErrorCode OptimizerRegistryManager::LoadAndSave(const std::string &key,
                                                  const std::string &id,
                                                  const Jsonizable *jsonizable) {
    std::map<std::string, std::string> value_map;
    auto ec = storage_->Load(key, value_map);
    if (ec != EC_OK && ec != EC_NOENT) {
        KVCM_LOG_ERROR("OptimizerRegistryManager: Load %s failed, id[%s], ec[%d]",
                       key.c_str(), id.c_str(), static_cast<int>(ec));
        return ec;
    }
    if (jsonizable) {
    ...
    if (ec != EC_OK) {
        KVCM_LOG_ERROR("OptimizerRegistryManager: Save %s failed, id[%s], ec[%d]",
                       key.c_str(), id.c_str(), static_cast<int>(ec));
        return ec;
    }
    return EC_OK;
...
std::vector<std::shared_ptr<const OptimizerInstanceGroup>>
OptimizerRegistryManager::ListInstanceGroups() const {
    std::shared_lock lock(mutex_);
...
ErrorCode OptimizerRegistryManager::SaveInstanceInfo(const OptimizerInstanceInfo &instance_info) {
    ...
    KVCM_LOG_INFO("OptimizerRegistryManager: saved instance info[%s] group[%s]",
                  instance_id.c_str(), instance_info.instance_group_name().c_str());
    return EC_OK;
}
...
ErrorCode OptimizerRegistryManager::LoadRecoveryData(RecoveryData &data) {
    ...
    for (const auto &[instance_id, json] : info_map) {
        ...
        instance_infos_[instance_id] = info;
    }

    KVCM_LOG_INFO("OptimizerRegistryManager: loaded %zu groups, %zu instances for recovery",
                  data.instance_groups.size(), data.instance_infos.size());
    return EC_OK;
}
