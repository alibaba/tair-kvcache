...
bool OptimizerInstanceInfo::FromRapidValue(const rapidjson::Value &rapid_value) {
    ...
    KVCM_JSON_GET_DEFAULT_MACRO(rapid_value, "linear_step", linear_step_, int32_t(1));
    KVCM_JSON_GET_DEFAULT_MACRO(rapid_value, "full_group_name", full_group_name_, std::string(""));
    return true;
}
...
void OptimizerInstanceInfo::ToRapidWriter(rapidjson::Writer<rapidjson::StringBuffer> &writer) const noexcept {
    ...
    Put(writer, "full_group_name", full_group_name_);
}
