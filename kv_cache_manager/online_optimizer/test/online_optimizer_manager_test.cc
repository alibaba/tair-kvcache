...
protected:
    ...
    OptimizerInstanceInfo MakeInfo(const std::string &instance_id = "i1",
        ...
        return OptimizerInstanceInfo(
            group_name, instance_id, block_size, MakeSpecs(), {}, linear_step, full_group_name);
    }
    ...
    OptimizerInstanceInfo MakeHybridInfo(const std::string &instance_id = "i1",
                                         ...
                                         const std::string &full_group_name = "") {
        return OptimizerInstanceInfo(
            group_name, instance_id, block_size, MakeHybridSpecs(), MakeHybridGroups(), linear_step, full_group_name);
    }
...
TEST_F(OnlineOptimizerManagerTest, ReRegisterFailurePreservesOldRecord) {
    ...
    auto group = MakeGroup();
    RegisterInstanceResult result;
    EXPECT_EQ(EC_OK, mgr->RegisterInstance(info, group, result));
    ...
    EXPECT_EQ(EC_BADARGS, mgr->RegisterInstance(bad_info, group, result2));
