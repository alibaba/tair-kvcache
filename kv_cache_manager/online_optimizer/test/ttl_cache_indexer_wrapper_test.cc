...
static std::unique_ptr<CacheIndexer>
MakeInnerIndexer(const std::string &type, int64_t max_key_count = 0, double capacity_gb = 10.0) {
...
TEST_F(TtlCacheIndexerWrapperTest, BasicExpiration) {
    ...
    TtlCacheIndexerWrapper wrapper(MakeInnerIndexer("fenwick_lru"), 10, clock);
...
TEST_F(TtlCacheIndexerWrapperTest, TtlAndCapacityEvictionInteraction) {
    int64_t now = 1000;
    auto clock = [&now]() { return now; };
    // max_key_count=3, capacity=2GB (2 blocks < 3, so max_key_count stays 3)
    TtlCacheIndexerWrapper wrapper(MakeInnerIndexer("fenwick_lru", 3, 2.0), 100, clock);
...
TEST_F(TtlCacheIndexerWrapperTest, WorksWithBstIndexer) {
    int64_t now = 1000;
    auto clock = [&now]() { return now; };
    TtlCacheIndexerWrapper wrapper(MakeInnerIndexer("bst_lru"), 10, clock);
...
TEST_F(TtlCacheIndexerWrapperTest, WorksWithLruIndexer) {
    int64_t now = 1000;
    auto clock = [&now]() { return now; };
    TtlCacheIndexerWrapper wrapper(MakeInnerIndexer("lru"), 10, clock);
