...
TEST_F(LruCacheIndexerTest, PostQueryMaintenanceUpdatesEviction) {
    // max_key_count = 100 so that mkc_bytes(100MB) > max_cap_bytes causes
    // max_cache to be created with 100MB capacity, which will evict entries.
    // Actually: use max_key_count with mkc_bytes <= max_cap_bytes so max_cache
    // is NOT created, and PostQueryMaintenance falls back to caches_.back().
    constexpr int64_t kOneMB = 1024LL * 1024;
    // max_key_count = 5, capacity = 3.5MB, charge = 1MB
    // avg_bytes_per_block = 1MB, mkc_bytes = 5MB > 3.5MB -> max_cache created with 5.5MB
    // 5.5MB can hold ~5 keys, but we insert 10 -> max_cache evicts
    LruCacheIndexer indexer(5);
    ...
    // Insert 10 keys -> max_cache (5MB) should evict older entries
...
TEST_F(LruCacheIndexerTest, MaxKeyCountLimitsTracking) {
    // max_key_count = 3, use large enough capacity so eviction comes from max_cache
    LruCacheIndexer indexer(3);
    constexpr int64_t kOneGB = 1024LL * 1024 * 1024;
    indexer.Init({10.0}, kOneGB, kOneGB, 1);
    ...
    // Insert more than max_key_count keys
    for (int64_t i = 1; i <= 5; i++) {
        indexer.ProcessKeys({i}, hit_count, max_hit);
    }
    EXPECT_EQ(5, indexer.unique_count());
    ...
    indexer.PostQueryMaintenance();
    // After maintenance, unique_count should be limited by max_cache capacity
    EXPECT_LE(indexer.unique_count(), 5);
}

TEST_F(LruCacheIndexerTest, MaxKeyCountZeroMeansUnlimited) {
    LruCacheIndexer indexer(0);
    constexpr int64_t kOneGB = 1024LL * 1024 * 1024;
    indexer.Init({10.0}, kOneGB, kOneGB, 1);
...
TEST_F(LruCacheIndexerTest, MaxHitCountWithUnlimitedCapacity) {
    // max_key_count = 0 means unlimited -> max_cache is created with huge capacity
    LruCacheIndexer indexer(0);
    constexpr int64_t kOneGB = 1024LL * 1024 * 1024;
    indexer.Init({1.0}, kOneGB, kOneGB, 1);
