...
protected:
    ...
    ErrorCode
    RegisterTestInstance(const std::string &instance_id, std::vector<double> caps = {1.0}, int64_t ttl_seconds = 0) {
...
TEST_F(OptimizerMetricsReporterTest, ReportIntervalCapacityEfficiency) {
    ...
    // With fenwick_lru, max_hit_rate should be > 0 after repeated queries
    MetricsTags tags = {{"instance_id", "inst1"}};
    Gauge max_rate = registry_->GetGauge("trace_query_max_hit_rate", tags);
