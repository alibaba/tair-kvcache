...
protected:
    ...
    void CreateTestGroup(const std::string &group_name, double capacity_gb = 1.0, bool enabled = true) {
...
TEST_F(OptimizerServiceImplTest, CreateInstanceGroupUnsupportedIndexerType) {
    proto::optimizer::CreateInstanceGroupRequest req;
    req.set_trace_id("t1");
    auto *g = req.mutable_instance_group();
    g->set_name("bad_grp");
    g->set_enabled(true);
    g->add_capacity_gb(1.0);
    g->set_indexer_type("unsupported_type");
...
TEST_F(OptimizerServiceImplTest, UpdateInstanceGroupUnsupportedIndexerType) {
    ...
    g->set_enabled(true);
    g->add_capacity_gb(1.0);
    g->set_indexer_type("bad_indexer");
...
TEST_F(OptimizerServiceImplTest, RegisterInstanceUnsupportedIndexerType) {
    ...
    // Directly register via manager with a bad group to test factory nullptr path
    OptimizerInstanceInfo info("idx_grp", "inst_bad", 1024, {LocationSpecInfo("full", 1024)}, {});
    OptimizerInstanceGroup bad_group;
    bad_group.set_name("idx_grp");
    bad_group.set_enabled(true);
    bad_group.set_capacity_gb({1.0});
    bad_group.set_indexer_type("nonexistent_indexer");

    RegisterInstanceResult result;
    ErrorCode ec = manager_->RegisterInstance(info, bad_group, result);
