...
ErrorCode OnlineOptimizerManager::RegisterInstanceInternal(const OptimizerInstanceInfo &instance_info,
    ...
    if (full_group_name.empty()) {
        ...
        if (size_full_only <= 0) {
            KVCM_LOG_ERROR(
                "RegisterInstance failed: full_group[%s] not found or has unresolved spec_names for instance[%s]",
    ...
    auto indexer = CacheIndexerFactory::CreateCacheIndexer(instance_group.indexer_type(),
                                                           instance_group.max_key_count(),
                                                           capacity_gb,
                                                           size_full_only,
                                                           size_full_linear,
                                                           linear_step,
    ...
    if (!indexer) {
        KVCM_LOG_ERROR("RegisterInstance failed: unsupported indexer_type[%s] for instance[%s]",
                       instance_group.indexer_type().c_str(),
                       instance_id.c_str());
        return EC_BADARGS;
    }
...
ErrorCode OnlineOptimizerManager::ResetStats(const std::string &instance_id) {
    ...
    std::lock_guard<std::mutex> guard(state->mutex);
    auto new_indexer = CacheIndexerFactory::CreateCacheIndexer(state->instance_group->indexer_type(),
    ...
    if (!new_indexer) {
        KVCM_LOG_ERROR("ResetStats failed: unsupported indexer_type[%s] for instance[%s]",
                       state->instance_group->indexer_type().c_str(),
                       instance_id.c_str());
        return EC_ERROR;
    }
