...
std::shared_ptr<OptimizerServiceMetricsCollector> MakeCollector(
    const std::shared_ptr<MetricsRegistry> &registry) {
...
#define REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(req_type, name, req_name, resp_name, method) \
    do {                                                                                        \
        Register##req_type##Handler(                                                            \
            "/api/" #name,                                                                      \
            GetHandler<OptimizerServiceHttp,                                                    \
                       proto::optimizer::req_name##Request,                                     \
                       proto::optimizer::resp_name##Response>(                                  \
                &OptimizerServiceHttp::method));                                                \
    } while (0)

#define REGISTER_HTTP_HANDLER_COMMON_RESP(req_type, name, req_name, method)                     \
    do {                                                                                        \
        Register##req_type##Handler(                                                            \
            "/api/" #name,                                                                      \
            GetHandler<OptimizerServiceHttp,                                                    \
                       proto::optimizer::req_name##Request,                                     \
                       proto::optimizer::CommonResponse>(                                       \
                &OptimizerServiceHttp::method));                                                \
    } while (0)
...
void OptimizerServiceHttp::RegisterHandler() {
    ...
    REGISTER_HTTP_HANDLER_COMMON_RESP(
        Post, optimizer/createInstanceGroup, CreateInstanceGroup, CreateInstanceGroup);
    REGISTER_HTTP_HANDLER_COMMON_RESP(
        Post, optimizer/updateInstanceGroup, UpdateInstanceGroup, UpdateInstanceGroup);
    REGISTER_HTTP_HANDLER_COMMON_RESP(
        Post, optimizer/removeInstanceGroup, RemoveInstanceGroup, RemoveInstanceGroup);
    REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(
        Post, optimizer/getInstanceGroup, GetInstanceGroup, GetInstanceGroup, GetInstanceGroup);
    REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(
        Post, optimizer/listInstanceGroups, ListInstanceGroups, ListInstanceGroups, ListInstanceGroups);
    ...
    // TraceQuery
    REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(
        Post, optimizer/traceQuery, TraceQuery, TraceQuery, TraceQuery);
    REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(
        Post, optimizer/listInstances, OptimizerListInstances, OptimizerListInstances, ListInstances);
    REGISTER_HTTP_HANDLER_FOR_OPTIMIZER_SERVICE(
        Post, optimizer/resetStats, OptimizerResetStats, OptimizerResetStats, ResetStats);
}
