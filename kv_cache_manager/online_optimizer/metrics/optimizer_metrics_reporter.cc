...
void OptimizerMetricsReporter::ReportInterval() {
    ...
    for (const auto &s : summaries) {
        ...
        Gauge query_total = metrics_registry_->GetGauge(prefix_ + ".trace_query_total", prom_tags);
        ...
        Gauge avg_bytes = metrics_registry_->GetGauge(prefix_ + ".trace_query_avg_bytes_per_block", prom_tags);
...
void OptimizerMetricsReporter::ReportPerQuery(MetricsCollector *collector) {
    ...
    if (metrics_registry_ && p->total_blocks() > 0) {
        ...
        for (const auto &info : per_cap) {
            ...
            Gauge hit_gauge = metrics_registry_->GetGauge(prefix_ + ".query_hit_count", prom_tags);
