...
```json
{
    ...
    "quota_planner_pools": [{
        ...
        "min_quota_transfer_bytes": 8589934592,
        "stability_required_plans": 3,
        "stability_tolerance_bytes": 8589934592,
...
收益保护按 pool 配置：DP 用 `理论命中率 - 迁移惩罚` 选方案，再校验总收益、每 TiB 转移收益、最小转移量和连续稳定轮数；任一条件不满足只发布 `FROZEN/NOOP`，不会触发硬配额。为兼容旧配置，收益阈值和迁移惩罚默认 0、稳定轮数默认 1，开启真实 resize 时应显式配置。`capacity_saving_sla_ratio` 仅计算达到各成员相对理论上限所需容量及可节省容量，不参与分桶或硬配额动作。
