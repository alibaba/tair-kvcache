...
std::vector<std::shared_ptr<OptimizerSchemaTrace>> StandardTraceLoader::LoadFromFile(const std::string &trace_file_path,
    ...
    while (std::getline(file, line)) {
        ...
        auto trace = ParseLine(line, fail);
        const bool is_request = dynamic_cast<const RequestSchemaTrace *>(trace.get()) != nullptr;
        if (replay_mode == TraceReplayMode::REQUEST && !is_request) {
            fail("trace_replay.mode=request only accepts type=request");
        }
        if (replay_mode == TraceReplayMode::READ_WRITE && is_request) {
