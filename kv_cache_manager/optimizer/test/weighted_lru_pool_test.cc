...
public:
    NaiveWeightedLru(uint64_t full_charge, uint64_t linear_charge) : charges_{full_charge, linear_charge} {}
...
TEST_F(WeightedLruPoolTest, TypedKeysDoNotCollide) {
    WeightedLruPool pool(/*full=*/100, /*linear=*/30);
    const CacheObjectKey full_key{CacheObjectType::kFull, 42};
    const CacheObjectKey linear_key{CacheObjectType::kLinear, 42};

    pool.Touch(full_key);
    EXPECT_TRUE(pool.IsResident(full_key));
    EXPECT_FALSE(pool.IsResident(linear_key));

    pool.Touch(linear_key);
    EXPECT_EQ(1u, pool.resident_full_count());
    EXPECT_EQ(1u, pool.resident_linear_count());
    EXPECT_EQ(130u, pool.resident_bytes());

    // Linear was touched later, so Full requires both charges, Linear only its own.
    uint64_t required = 0;
    ASSERT_TRUE(pool.RequiredBytes(linear_key, required));
...
TEST_F(WeightedLruPoolTest, RandomizedOracleComparison) {
    ...
    WeightedLruPool pool(/*full=*/48, /*linear=*/16);
    NaiveWeightedLru oracle(48, 16);

    std::uniform_int_distribution<int64_t> key_dist(0, 63);
    std::uniform_int_distribution<int> type_dist(0, 4);
    for (int step = 0; step < 20000; ++step) {
        const CacheObjectKey key{type_dist(rng) == 0 ? CacheObjectType::kLinear : CacheObjectType::kFull,
                                 key_dist(rng)};
        pool.Touch(key);
        oracle.Touch(key);

        if (step % 7 == 0) {
            const CacheObjectKey probe{type_dist(rng) == 0 ? CacheObjectType::kLinear : CacheObjectType::kFull,
                                       key_dist(rng)};
            uint64_t expected = 0;
            uint64_t actual = 0;
...
TEST_F(WeightedLruPoolTest, CompactionPreservesOrderAndBytes) {
    ...
    for (int round = 0; round < 3000; ++round) {
        for (int64_t key = 0; key < 8; ++key) {
            const CacheObjectKey object{key % 3 == 0 ? CacheObjectType::kLinear : CacheObjectType::kFull, key};
            pool.Touch(object);
            oracle.Touch(object);
        }
    }
    pool.MaybeCompactPositions();
    for (int64_t key = 0; key < 8; ++key) {
        const CacheObjectKey object{key % 3 == 0 ? CacheObjectType::kLinear : CacheObjectType::kFull, key};
        uint64_t expected = 0;
        uint64_t actual = 0;
