...
void OptimizerServiceImpl::RegisterInstance(RequestContext *request_context,
    ...
    if (ec == EC_OK) {
        ...
        // new labels (e.g. different capacity_gb tiers).
        ...
        // the byte size of one Full block plus one Linear state.
        response->set_size_full(result.full_charge_bytes);
        response->set_size_full_linear(result.full_charge_bytes + result.linear_charge_bytes);
    }
}
