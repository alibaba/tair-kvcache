...
int main(int argc, char *argv[]) {
    // 1. 初始化日志（支持 ALOG_CONF 环境变量覆盖配置文件路径）
    //    若指定配置不存在则回退到默认配置（输出到 logs/kv_cache_manager.log）
    const char *alog_conf_env = std::getenv("ALOG_CONF");
    std::string alog_conf_path =
        alog_conf_env ? std::string(alog_conf_env) : "kv_cache_manager/tools/v6d_benchmark/benchmark_alog.conf";
    if (!LoggerBroker::InitLogger(alog_conf_path, false)) {
        std::cerr << "[WARN] Failed to init logger with config: " << alog_conf_path
                  << ", falling back to default logger" << std::endl;
        LoggerBroker::InitLogger("", false);
    }
