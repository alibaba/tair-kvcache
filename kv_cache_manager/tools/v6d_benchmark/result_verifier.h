...
inline ResultVerifier::VerificationResult ResultVerifier::VerifyQuery(const std::vector<int64_t> &query_keys,
    ...
    if (result.success) {
        ...
        if (!result.missing_keys.empty()) {
            KVCM_LOG_DEBUG("Verification failed: %zu missing keys (first 5: %s)",
                           result.missing_keys.size(),
                           result.missing_keys.size() > 0 ? std::to_string(result.missing_keys[0]).c_str() : "N/A");
        }
        if (!result.unexpected_keys.empty()) {
            KVCM_LOG_DEBUG("Verification failed: %zu unexpected keys (first 5: %s)",
                           result.unexpected_keys.size(),
                           result.unexpected_keys.size() > 0 ? std::to_string(result.unexpected_keys[0]).c_str()
                                                             : "N/A");
        }
    }
