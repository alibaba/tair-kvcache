 ...
 */
public class CacheAwareGrpcTest extends IntegrationTestBase {
    ...
    void testGetCacheLocationsByBackend_basicQuery() {
        ...
        GetCacheLocationsByBackendResponse response = grpcClient.getCacheLocationsByBackend(request);
    ...
    void testGetCacheLocationsByBackend_instanceNotExist() {
        ...
        assertThrows(Exception.class, () -> grpcClient.getCacheLocationsByBackend(request));
    }
    ...
    @Test
    void testGetCacheLocationLen_prefixMatchWithBreak() {
        // Setup: write keys [1, 2, 3]
        registerInstance(instanceId);
        String sessionId = startWriteCache(instanceId, 1L, 2L, 3L);
        finishWriteCache(instanceId, sessionId, 3);

        // Query with QT_PREFIX_MATCH: [1, 2, 3, 8, 5, 6]
        GetCacheLocationLenRequest request = GetCacheLocationLenRequest.newBuilder()
                ...
                .addBlockKeys(2L)
                .addBlockKeys(3L)
                .addBlockKeys(8L)  // Break here
                .addBlockKeys(5L)
                .addBlockKeys(6L)
                .build();

        GetCacheLocationLenResponse response = grpcClient.getCacheLocationLen(request);

        // Verify: should return 3 (prefix breaks at key 8)
        assertEquals(3, response.getCacheLocationLen());
    }

    @Test
    void testGetCacheLocationLen_prefixMatchNoMatches() {
        registerInstance(instanceId);

        // Query with QT_PREFIX_MATCH: [999, 1, 2] (first key doesn't exist)
        GetCacheLocationLenRequest request = GetCacheLocationLenRequest.newBuilder()
                .setTraceId("test-trace")
                .setInstanceId(instanceId)
                .setQueryType(QueryType.QT_PREFIX_MATCH)
    ...
    @Test
    void testGetCacheLocationLen_batchGetWithExistingKeys() {
        // Setup: write keys [1, 2, 3, 4, 5, 6, 7] with offset=5
        // This means keys [1, 2, 3, 4, 5] are written successfully
        registerInstance(instanceId);
        String sessionId = startWriteCache(instanceId, 1L, 2L, 3L, 4L, 5L, 6L, 7L);
        finishWriteCacheWithOffset(instanceId, sessionId, 5);

        // Query for keys [1, 2, 8, 4, 5, 9, 6]
        GetCacheLocationLenRequest request = GetCacheLocationLenRequest.newBuilder()
                .setTraceId("test-trace")
                .setInstanceId(instanceId)
                .setQueryType(QueryType.QT_BATCH_GET)
        ...
        // Verify: keys 1, 2, 4, 5 exist (4 keys)
    ...
    @Test
    void testGetCacheLocationLen_consistencyWithGetCacheLocation() {
        // Setup: write keys [1, 2, 3, 4, 5]
        registerInstance(instanceId);
        String sessionId = startWriteCache(instanceId, 1L, 2L, 3L, 4L, 5L);
        finishWriteCache(instanceId, sessionId, 5);

        // Query with GetCacheLocation
        ...
        GetCacheLocationResponse locationResponse = grpcClient.getCacheLocation(locationRequest);

        // Query with GetCacheLocationLen
        ...
        GetCacheLocationLenResponse lenResponse = grpcClient.getCacheLocationLen(lenRequest);

        // Verify: count matches number of locations
    ...
    void testGetCacheMeta_servingStatus() {
        ...
        GetCacheMetaResponse response = grpcClient.getCacheMeta(request);
    ...
    void testGetCacheMeta_instanceNotExist() {
        ...
        assertThrows(Exception.class, () -> grpcClient.getCacheMeta(request));
    }
