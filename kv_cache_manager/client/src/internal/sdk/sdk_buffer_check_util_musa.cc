...
std::vector<uint32_t> SdkBufferCheckUtil::GetIovsCrc(const std::vector<IovDevice> &iovs_h) {
    ...
    if (!iovs_guard.Alloc(sizeof(IovDevice) * iovs_h.size()) || !crcs_guard.Alloc(sizeof(uint32_t) * iovs_h.size())) {
        return {};
    }
    return GetIovsCrc(
...
SdkBufferCheckPool::~SdkBufferCheckPool() {
    ...
    if (device_id_ >= 0) {
        musaError_t err = musaGetDevice(&previous_device_id);
        if (err != musaSuccess) {
            KVCM_LOG_WARN("musa error [%d] [%s] | musaGetDevice before checksum pool cleanup failed",
                          err,
                          musaGetErrorString(err));
        } else if (previous_device_id != device_id_) {
            err = musaSetDevice(device_id_);
            if (err != musaSuccess) {
                KVCM_LOG_WARN("musa error [%d] [%s] | musaSetDevice [%d] before checksum pool cleanup failed",
                              err,
                              musaGetErrorString(err),
                              device_id_);
            } else {
    ...
    if (restore_device) {
        CHECK_MUSA_ERROR(musaSetDevice(previous_device_id),
                         "musaSetDevice prev [%d] after checksum pool cleanup failed",
                         previous_device_id);
    }
}
...
bool SdkBufferCheckPool::WarmUp() {
    ...
    // crc32("12") == 4F5344CD
    MusaBufferGuard buffer_guard;
    const auto byte_size = warmup_str.size();
    if (!buffer_guard.Alloc(byte_size)) {
        return false;
    }
    void *buffer = buffer_guard.Get();
    CHECK_MUSA_ERROR_RETURN(musaMemcpy(buffer, warmup_str.data(), byte_size, musaMemcpyHostToDevice),
                            false,
                            "musaMemcpy from host[%p] to device[%p] failed",
                            warmup_str.data(),
                            buffer);
    const std::vector<IovDevice> iovs_h{{buffer, byte_size}};
    const std::vector<uint32_t> expected_crcs({0x4F5344CD});
    for (auto &cell : cells_) {
        const auto real_crcs = SdkBufferCheckUtil::GetIovsCrc(iovs_h, cell.d_iovs, cell.d_crcs, cell.gpu_stream);
        if (expected_crcs != real_crcs) {
            KVCM_LOG_ERROR("warm up failed");
            return false;
        }
    }
    return buffer_guard.Free();
}
