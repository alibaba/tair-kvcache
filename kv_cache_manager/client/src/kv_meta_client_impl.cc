...
bool IsRetryable(ClientErrorCode error) { return error == ER_SERVICE_NOT_LEADER || error == ER_SERVICE_NOT_READY; }
...
ClientErrorCode KvMetaClientImpl::Call(Response *response, TransportRetryPolicy transport_retry_policy, Rpc &&rpc) {
...
std::pair<ClientErrorCode, KvMetaInstanceInfo> KvMetaClientImpl::GetInstanceInfo(const std::string &trace_id) {
    ...
    return {ER_OK, {source.quota_group_name(), source.instance_group_name(), source.instance_id()}};
}

std::pair<ClientErrorCode, KvMetaGetResult> KvMetaClientImpl::Get(const std::string &trace_id,
