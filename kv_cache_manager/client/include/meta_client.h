...
public:
    ...
    // API v4: appended after every v3 slot. The separate options type avoids
    // changing the size of the released FinishWriteOptions object.
    virtual ClientErrorCode FinishWriteWithIntegrity(const std::string &trace_id,
                                                     const std::string &write_session_id,
                                                     const BlockMask &success_block,
                                                     const Locations &locations,
                                                     const FinishWriteIntegrityOptions &options) {
        if (!options.uri_batches.empty()) {
            return ER_CHECKSUM_UNAVAILABLE;
        }
        return FinishWrite(trace_id,
                           write_session_id,
                           success_block,
                           locations,
                           FinishWriteOptions::WithChecksumBatches(options.checksum_batches));
    }
};
} // namespace kv_cache_manager
