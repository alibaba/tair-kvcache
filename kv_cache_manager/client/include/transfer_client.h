...
public:
    ...
    static std::unique_ptr<TransferClient> Create(const std::string &client_config,
    ...
    ClientErrorCode LoadKvCaches(const UriStrVec &uri_str_vec, const BlockBuffers &block_buffers, std::nullptr_t) {
...
public:
    ...
    // Compute the exact checksum batch used by SaveKvCaches/LoadKvCaches
    // without performing storage I/O. This virtual is appended so callers at
    // an integration boundary can take an independent snapshot, then ask KVCM
    // to recompute and validate the same buffers during the actual transfer.
    // Legacy implementations fail explicitly instead of silently returning an
    // empty batch.
    virtual std::pair<ClientErrorCode, std::vector<int64_t>> CalculateChecksums(const BlockBuffers &) {
        return {ER_CHECKSUM_UNAVAILABLE, {}};
    }

    // Report the implementation used by CalculateChecksums and the
    // checksum-aware Save/Load overloads. The fallback keeps legacy
    // subclasses source-compatible while capability negotiation fails closed.
    virtual ChecksumCapability GetChecksumCapability() const { return {}; }
};
} // namespace kv_cache_manager
