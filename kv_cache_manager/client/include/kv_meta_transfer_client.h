...
// not require every value size to be registered as a fixed location spec. Its
// client config instead carries the sole schema marker {"value": 1}; actual
// object sizes are supplied separately to LoadObjects/SaveObjects.
class KvMetaTransferClient {
public:
    virtual ~KvMetaTransferClient() = default;

    static std::unique_ptr<KvMetaTransferClient>
    Create(const std::string &client_config, const InitParams &init_params, std::uint64_t max_object_bytes);
    ...
    virtual std::pair<ClientErrorCode, UriStrVec> SaveObjects(const UriStrVec &uri_str_vec,
                                                              const std::vector<std::uint64_t> &value_sizes,
                                                              const BlockBuffers &object_buffers) = 0;
