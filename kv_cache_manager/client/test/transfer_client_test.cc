...
// FinishWriteOptions crossed the released v3 C++ ABI. API v4 must add its URI
// payload in a separate type instead of growing the object passed by reference.
static_assert(sizeof(FinishWriteOptions) == sizeof(std::vector<LocationSpecChecksumBatch>));
static_assert(alignof(FinishWriteOptions) == alignof(std::vector<LocationSpecChecksumBatch>));

TEST(TransferClientApiTest, ExposesStagedChecksumRuntimeApiVersion) {
    EXPECT_EQ(static_cast<uint32_t>(KVCM_STAGED_CHECKSUM_API_VERSION), KVCMStagedChecksumRuntimeApiVersion());
}

TEST(TransferClientApiTest, ChecksumWorkloadBudgetMirrorsHeadTailSampling) {
    BlockBuffers blocks(2);
    blocks[0].iovs.push_back({MemoryType::GPU, reinterpret_cast<void *>(1), 9, false});
    blocks[1].iovs.push_back({MemoryType::GPU, reinterpret_cast<void *>(2), 2, false});
    blocks[1].iovs.push_back({MemoryType::GPU, reinterpret_cast<void *>(3), std::numeric_limits<size_t>::max(), false});

    // sample=4 reads 8 bytes from the odd-sized iov (center omitted), 2 bytes
    // from the tiny iov, and 8 bytes from the huge iov.
    EXPECT_TRUE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 18));
    EXPECT_FALSE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 17));

    EXPECT_TRUE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 8, {true, false}));
    EXPECT_FALSE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 7, {true, false}));
    EXPECT_FALSE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 18, {true}));
    EXPECT_FALSE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 0, 18));
    EXPECT_FALSE(TransferClientImpl::IsChecksumWorkloadWithinLimit(blocks, 4, 0));
}
...
TEST(ClientOptionsTest, LegacyOnlySubclassesUseSafeOptionFallbacks) {
    ...
    EXPECT_EQ(1, transfer.load_calls);
    EXPECT_EQ(ER_OK,
              transfer.LoadKvCaches(uris, buffers, LoadKvCachesOptions::VerifyWith({0}, std::vector<bool>{false})));
    EXPECT_EQ(2, transfer.load_calls);
    EXPECT_EQ(ER_CHECKSUM_MISMATCH,
              transfer.LoadKvCaches(uris, buffers, LoadKvCachesOptions::VerifyWith(std::vector<int64_t>{})));
    EXPECT_EQ(2, transfer.load_calls);
    ...
    EXPECT_EQ(
        ...
        transfer.SaveKvCaches(uris, buffers, SaveKvCachesOptions::VerifyCallerChecksums(std::vector<int64_t>{})).first);
    EXPECT_EQ(1, transfer.save_calls);
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE, transfer.CalculateChecksums(buffers).first);
    EXPECT_FALSE(transfer.GetChecksumCapability().available);
    ...
    EXPECT_EQ(1, meta.finish_calls);
    EXPECT_EQ(ER_OK,
              meta.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, FinishWriteIntegrityOptions{}));
    EXPECT_EQ(2, meta.finish_calls);
    FinishWriteIntegrityOptions meta_checksum_only;
    meta_checksum_only.checksum_batches = {{"tp0", checksums}};
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE,
              meta.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, meta_checksum_only));
    EXPECT_EQ(2, meta.finish_calls);
    FinishWriteIntegrityOptions meta_with_uri;
    meta_with_uri.uri_batches = {{"tp0", {"file://test"}, {true}}};
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE,
              meta.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, meta_with_uri));
    EXPECT_EQ(2, meta.finish_calls);
    ...
    EXPECT_EQ(
        ER_OK,
        manager.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, FinishWriteIntegrityOptions{}));
    EXPECT_EQ(2, manager.finish_calls);
    FinishWriteIntegrityOptions manager_checksum_only;
    manager_checksum_only.checksum_batches = {{"tp0", checksums}};
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE,
              manager.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, manager_checksum_only));
    EXPECT_EQ(2, manager.finish_calls);
    FinishWriteIntegrityOptions manager_with_uri;
    manager_with_uri.uri_batches = {{"tp0", {"file://test"}, {true}}};
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE,
              manager.FinishWriteWithIntegrity(trace_id, "session", block_mask, locations, manager_with_uri));
    EXPECT_EQ(2, manager.finish_calls);
    ...
    EXPECT_EQ(ER_OK,
              manager.LoadKvCaches(uris, buffers, LoadKvCachesOptions::VerifyWith({0}, std::vector<bool>{false})));
    EXPECT_EQ(2, manager.load_calls);
    EXPECT_EQ(ER_CHECKSUM_MISMATCH,
              manager.LoadKvCaches(uris, buffers, LoadKvCachesOptions::VerifyWith(std::vector<int64_t>{})));
    EXPECT_EQ(2, manager.load_calls);
    ...
    EXPECT_EQ(
        ER_CHECKSUM_MISMATCH,
        manager.SaveKvCaches(uris, buffers, SaveKvCachesOptions::VerifyCallerChecksums(std::vector<int64_t>{})).first);
    EXPECT_EQ(1, manager.save_calls);
}

TEST(ClientOptionsTest, V4ChecksumOnlyFallbackUsesV3VirtualWithoutChangingLegacyOptions) {
    RecordingManagerClient client;
    ManagerClient &api = client;
    const BlockMask block_mask = static_cast<BlockMaskOffset>(0);
    const Locations locations = {{{"tp0", "file://test"}}};

    FinishWriteIntegrityOptions checksum_only;
    checksum_only.checksum_batches = {{"tp0", {0, -1}}};
    ASSERT_EQ(ER_OK, api.FinishWriteWithIntegrity("trace", "session", block_mask, locations, checksum_only));
    ...
    EXPECT_EQ((std::vector<int64_t>{0, -1}), client.last_finish_write_options.checksum_batches[0].checksums);

    FinishWriteIntegrityOptions with_uri = checksum_only;
    with_uri.uri_batches = {{"tp0", {"file://test"}, {true}}};
    EXPECT_EQ(ER_CHECKSUM_UNAVAILABLE,
              api.FinishWriteWithIntegrity("trace", "session", block_mask, locations, with_uri));
    ASSERT_EQ(1u, client.last_finish_write_options.checksum_batches.size());
    EXPECT_EQ((std::vector<int64_t>{0, -1}), client.last_finish_write_options.checksum_batches[0].checksums);
}
...
TEST_F(TransferClientTest, TestCreateAcceptsMetaChecksumSpec) {
    ...
    ASSERT_NE(client, nullptr);
#if defined(USING_CUDA) || defined(USING_MUSA)
    const auto capability = client->GetChecksumCapability();
    EXPECT_TRUE(capability.available);
    EXPECT_EQ(capability.algorithm_id, KVCM_CHECKSUM_ALGORITHM_CRC32_XOR_INT64_LEGACY_V0);
    EXPECT_GT(capability.sample_bytes, 0u);
#else
    EXPECT_FALSE(client->GetChecksumCapability().available);
#endif
    free(init_params.regist_span->base);
    delete init_params.regist_span;
}
...
#if defined(USING_CUDA) || defined(USING_MUSA)
TEST_F(TransferClientTest, TestCreateRejectsChecksumSampleSizeChangedAfterGpuImplementationLoaded) {
    const size_t kernel_sample_bytes = SdkBufferCheckUtil::ChecksumSampleBytes();
    ASSERT_GT(kernel_sample_bytes, 0u);
    const std::string changed_sample_bytes = kernel_sample_bytes == 1 ? "2" : "1";
    const char *previous_env = std::getenv("KVCM_CHECK_IOV_BYTE_SIZE");
    const std::optional<std::string> previous_value =
        previous_env == nullptr ? std::nullopt : std::optional<std::string>{previous_env};
    ASSERT_EQ(0, setenv("KVCM_CHECK_IOV_BYTE_SIZE", changed_sample_bytes.c_str(), 1));
    ...
    if (previous_value.has_value()) {
        ASSERT_EQ(0, setenv("KVCM_CHECK_IOV_BYTE_SIZE", previous_value->c_str(), 1));
    } else {
        ASSERT_EQ(0, unsetenv("KVCM_CHECK_IOV_BYTE_SIZE"));
    }
    EXPECT_EQ(client, nullptr);
    free(init_params.regist_span->base);
    delete init_params.regist_span;
...
TEST_F(TransferClientTest, TestCalculateChecksumsRejectsEmptyBatch) {
    auto client = TransferClient::Create(client_config_, init_params_);
    ASSERT_NE(client, nullptr);

    const auto result = client->CalculateChecksums({});

    EXPECT_EQ(ER_INVALID_PARAMS, result.first);
    EXPECT_TRUE(result.second.empty());
}

TEST_F(TransferClientTest, TestCalculateChecksumsFailsClosedWithoutChecksumPool) {
    auto client = TransferClient::Create(client_config_, init_params_);
    ASSERT_NE(client, nullptr);
    const BlockBuffers block_buffers = {BlockBuffer{}};

    const auto result = client->CalculateChecksums(block_buffers);
