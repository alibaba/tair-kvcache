...
#include "kv_cache_manager/common/cache/auto_vector.h"
#include "kv_cache_manager/common/cache/sharded_cache.h"
...
struct LRUHandle : public Cache::Handle {
    ...
    inline size_t CalcuMetaCharge(CacheMetadataChargePolicy metadata_charge_policy) const {
        if (metadata_charge_policy != kFullChargeCacheMetadata) {
            return 0;
        } else {
#ifdef ROCKSDB_MALLOC_USABLE_SIZE
            return malloc_usable_size(const_cast<void *>(static_cast<const void *>(this)));
#else
    ...
    inline void CalcTotalCharge(size_t charge, CacheMetadataChargePolicy metadata_charge_policy) {
        total_charge = charge + CalcuMetaCharge(metadata_charge_policy);
    }

    inline size_t GetCharge(CacheMetadataChargePolicy metadata_charge_policy) const {
...
public: // Function definitions expected as parameter to ShardedCache
    static inline HashVal ComputeHash(const std::string_view &key, uint32_t seed) {
        return Lower32of64(Hash64(key.data(), key.size(), seed));
    }
    ...
    LRUHandle *Lookup(const std::string_view &key,
                      uint32_t hash,
                      const Cache::CacheItemHelper *helper,
...
private:
    ...
    LRUHandle *CreateHandle(const std::string_view &key,
                            uint32_t hash,
                            Cache::ObjectPtr value,
                            const Cache::CacheItemHelper *helper,
                            size_t charge);
...
public:
    ...
    void ApplyToHandle(Cache *cache,
                       Handle *handle,
                       const std::function<void(
                           const std::string_view &key, ObjectPtr obj, size_t charge, const CacheItemHelper *helper)>
                           &callback) override;
