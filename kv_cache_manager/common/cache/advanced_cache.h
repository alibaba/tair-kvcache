...
public: // type defs
    ...
    using SaveToCallback = ErrorCode (*)(ObjectPtr from_obj, size_t from_offset, size_t length, char *out_buf);
    ...
    using CreateCallback = ErrorCode (*)(const std::string_view &data,
    ...
    struct CacheItemHelper {
        ...
        explicit CacheItemHelper(CacheEntryRole _role,
                                 DeleterFn _del_cb,
                                 SizeCallback _size_cb,
        ...
        inline bool IsSecondaryCacheCompatible() const { return size_cb != nullptr; }
    };
...
public: // ctor/dtor/create
    ...
    static ErrorCode
    CreateFromString(const ConfigOptions &config_options, const std::string &value, std::shared_ptr<Cache> *result);
...
public: // functions
    ...
    virtual ErrorCode Insert(const std::string_view &key,
                             ObjectPtr obj,
                             const CacheItemHelper *helper,
                             size_t charge,
                             Handle **handle = nullptr,
                             Priority priority = Priority::LOW,
                             const std::string_view &compressed = std::string_view(),
    ...
    virtual Handle *CreateStandalone(const std::string_view &key,
                                     ObjectPtr obj,
                                     const CacheItemHelper *helper,
                                     size_t charge,
    ...
    virtual ErrorCode GetSecondaryCacheCapacity(size_t & /*size*/) const { return EC_UNIMPLEMENTED; }

    virtual ErrorCode GetSecondaryCachePinnedUsage(size_t & /*size*/) const { return EC_UNIMPLEMENTED; }
    ...
    virtual void ApplyToHandle(
        ...
        Handle *handle,
        const std::function<void(
            const std::string_view &key, ObjectPtr obj, size_t charge, const CacheItemHelper *helper)> &callback) = 0;
    // Remove all entries.
    ...
    virtual bool Release(Handle *handle, bool /*useful*/, bool erase_if_last_ref) {
    ...
    struct AsyncLookupHandle {
        ...
        AsyncLookupHandle() {}
        AsyncLookupHandle(const std::string_view &_key,
                          const CacheItemHelper *_helper,
                          CreateContext *_create_context,
                          Priority _priority = Priority::LOW,
                          Statistics *_stats = nullptr)
            : key(_key), helper(_helper), create_context(_create_context), priority(_priority), stats(_stats) {}
    ...
    using EvictionCallback = std::function<bool(const std::string_view &key, Handle *h, bool was_hit)>;
...
public:
    explicit CacheWrapper(std::shared_ptr<Cache> target) : target_(std::move(target)) {}

    // Only function that derived class must provide
    // const char* Name() const override { ... }

    ErrorCode Insert(const std::string_view &key,
                     ObjectPtr value,
                     const CacheItemHelper *helper,
                     size_t charge,
                     Handle **handle = nullptr,
                     Priority priority = Priority::LOW,
                     const std::string_view &compressed_value = std::string_view(),
                     CompressionType type = CompressionType::kNoCompression) override {
        return target_->Insert(key, value, helper, charge, handle, priority, compressed_value, type);
    }
    ...
    Handle *Lookup(const std::string_view &key,
                   const CacheItemHelper *helper,
    ...
    bool HasStrictCapacityLimit() const override { return target_->HasStrictCapacityLimit(); }

    size_t GetOccupancyCount() const override { return target_->GetOccupancyCount(); }

    size_t GetTableAddressCount() const override { return target_->GetTableAddressCount(); }

    size_t GetCapacity() const override { return target_->GetCapacity(); }

    size_t GetUsage() const override { return target_->GetUsage(); }

    size_t GetUsage(Handle *handle) const override { return target_->GetUsage(handle); }

    size_t GetPinnedUsage() const override { return target_->GetPinnedUsage(); }

    size_t GetCharge(Handle *handle) const override { return target_->GetCharge(handle); }
