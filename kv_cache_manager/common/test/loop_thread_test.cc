...
TEST_F(LoopThreadTest, TestCreateAndStop) {
    ...
    // Poll until the loop thread fires at least once, with a generous timeout
    // to avoid flakes on busy CI/builders where a fixed 10ms sleep is not
    // enough for the worker thread to be scheduled even once.
    ...
    // After Stop(), let any in-flight iteration finish, then verify the
    // counter is stable (no further increments).
