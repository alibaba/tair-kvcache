...
std::vector<SpecCreateResult> DummyBackend::Create(const CreateBlocksRequest &request,
    ...
    for (const auto &spec_block : request.spec_block_keys) {
        ...
        for (size_t start = 0; start < total_key_count; start += batch_size) {
            size_t end = std::min(start + static_cast<size_t>(batch_size), total_key_count);
            ...
            for (size_t j = 0; j < batch_keys.size(); ++j) {
                ...
                spec_result.emplace_back(ErrorCode::EC_OK, storage_uri);
            }
        }
