...
inline bool TryGetExactTairMempoolOffset(const DataStorageUri &uri, std::uint64_t &offset) noexcept {
    ...
    if (parsed.ec != std::errc{} || parsed.ptr != end || (end - begin > 1 && *begin == '0')) {
        return false;
    }
...
inline bool HasExactTairMempoolOffset(const DataStorageUri &uri) noexcept {
    ...
    return TryGetExactTairMempoolOffset(uri, offset) && offset != 0;
}

inline bool TryGetTairMempoolUint16Param(const DataStorageUri &uri,
    ...
    if (text.empty() || parsed.ec != std::errc{} || parsed.ptr != text.data() + text.size() ||
        text != std::to_string(parsed_value)) {
        return false;
    }
    value = parsed_value;
    return true;
...
inline bool TryGetTairMempoolUint16ParamOrDefault(const DataStorageUri &uri,
                                                   const std::string &name,
                                                   std::uint16_t &value) {
    return TryGetTairMempoolUint16Param(uri, name, value, false);
}

// media_type=0 is the deployed PACE "unspecified/automatic" request policy,
// not a physical tier. Depending on Provider mode, a successful allocation
// may report either 0 (transparent tiering) or its concrete DRAM/SSD tier.
// Explicit DRAM/SSD registrations remain strict so a response cannot silently
// cross the configured physical pool.
inline constexpr bool IsKvMetaTairMempoolMediaCompatible(std::uint16_t configured,
                                                          std::uint16_t actual) noexcept {
...
inline bool HasCanonicalRequiredTairMempoolUint16Param(const DataStorageUri &uri, const std::string &name) {
    std::uint16_t ignored = 0;
    return TryGetTairMempoolUint16Param(uri, name, ignored, true);
}

// KVCM's existing PACE adapter always emits these three fields. Requiring
// their canonical spelling prevents malformed text from silently becoming
// zero in the data-plane SDK and keeps media-pool validation deterministic.
inline bool HasExactTairMempoolAddress(const DataStorageUri &uri) {
    std::uint16_t node_id = 0;
    return HasExactTairMempoolOffset(uri) && TryGetTairMempoolUint16Param(uri, "node_id", node_id, true) &&
           node_id != 0 && HasCanonicalRequiredTairMempoolUint16Param(uri, "media_type") &&
           HasCanonicalRequiredTairMempoolUint16Param(uri, "range_id");
}

// Validate the backend fields that make one URI an independently deletable
// KVMeta allocation. TairMempool uses its existing opaque GA identity; KVCM
// deliberately does not require Provider generation fields that are absent
// from the deployed PACE protocol.
...
inline bool UriMatchesConfiguredKvMetaNamespace(const DataStorageUri &uri,
    ...
    if (IsTairMempoolStorageType(storage_type)) {
        // PACE addresses are allocator-owned opaque offsets, and media_type is
        // part of their physical pool identity. An unspecified registration
        // accepts only PACE's supported automatic/concrete results; an
        // explicit SSD/DRAM registration must return exactly that tier.
        ...
        return spec && TryGetTairMempoolUint16ParamOrDefault(uri, "media_type", uri_media_type) &&
               IsKvMetaTairMempoolMediaCompatible(spec->media_type(), uri_media_type);
    }
...
inline bool HasSafeConfiguredKvMetaNamespace(const StorageConfig &config,
    ...
    case DataStorageType::DATA_STORAGE_TYPE_NFS:
    case DataStorageType::DATA_STORAGE_TYPE_DUMMY: {
        if (config.type() == DataStorageType::DATA_STORAGE_TYPE_NFS) {
            const auto spec = std::dynamic_pointer_cast<NfsStorageSpec>(config.storage_spec());
            // NfsBackend intentionally preserves its legacy raw string
            // concatenation.  Without a trailing separator, `root` plus
            // `kvmeta/...` escapes the configured directory as
            // `rootkvmeta/...`.  Reject only KVMeta admission here so the
            // established fixed-block configuration contract is unchanged.
            if (!spec || spec->root_path().empty() || spec->root_path().back() != '/') {
                return false;
            }
        }
