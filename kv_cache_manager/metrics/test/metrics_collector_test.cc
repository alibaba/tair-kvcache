...
TEST_F(MetricsCollectorTest, MetaIndexerMetricsTest) {
    ...
    EXPECT_DOUBLE_EQ(GET(p, meta_indexer, lock_hold_time_us), 0.);
    ...
    SET_METRICS_(p, meta_indexer, lock_hold_time_us, 300.);
    ...
    EXPECT_DOUBLE_EQ(GET(p, meta_indexer, lock_hold_time_us), 300.);
...
TEST_F(MetricsCollectorTest, ServiceMetricsTest) {
    ...
    latency.request_parse_time_us = 10;
    latency.service_callback_time_us = 170;
    latency.response_serialize_time_us = 30;
    latency.handler_time_us = 215;
    latency.request_receive_wait_time_us = 200;
    latency.io_event_loop_lag_us = 15;
    latency.response_build_time_us = 8;
    latency.socket_write_time_us = 9;
    p->RecordHttpRequestLatency(latency);
    ...
    EXPECT_EQ(GET(p, http, service_callback_time_us_sum), 170);
    EXPECT_EQ(GET(p, http, response_serialize_time_us_sum), 30);
    EXPECT_EQ(GET(p, http, handler_time_us_sum), 215);
    ...
    latency = {};
    latency.request_parse_time_us = 1;
    latency.service_callback_time_us = 2;
    latency.response_serialize_time_us = 3;
    latency.handler_time_us = 7;
    latency.request_receive_wait_time_us = 4;
    latency.io_event_loop_lag_us = 5;
    latency.response_build_time_us = 6;
    latency.socket_write_time_us = 7;
    p->RecordHttpRequestLatency(latency);
