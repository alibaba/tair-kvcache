...
public:
    ...
    void BlockNextLocationMapReadAfterRead() {
        std::lock_guard<std::mutex> lock(control_mutex_);
        block_next_location_map_read_after_read_ = true;
        location_map_read_after_read_entered_ = false;
        release_location_map_read_after_read_ = false;
    }

    bool WaitUntilLocationMapReadAfterReadEntered(std::chrono::milliseconds timeout) {
        std::unique_lock<std::mutex> lock(control_mutex_);
        return control_cv_.wait_for(lock, timeout, [&] { return location_map_read_after_read_entered_; });
    }

    void ReleaseLocationMapReadAfterRead() {
        std::lock_guard<std::mutex> lock(control_mutex_);
        release_location_map_read_after_read_ = true;
    ...
    void FailSyncCall(size_t call_index) {
        std::lock_guard<std::mutex> lock(control_mutex_);
        fail_sync_call_ = call_index;
        sync_call_count_ = 0;
    }
    ...
    std::vector<ErrorCode> GetLocations(RequestContext *request_context,
        ...
        MaybeBlockLocationMapReadAfterRead();
        return results;
    }
    ...
    bool Sync(const KeyTypeVec &keys) noexcept override {
        bool fail = false;
        {
            std::lock_guard<std::mutex> lock(control_mutex_);
            ++sync_call_count_;
            fail = sync_call_count_ == fail_sync_call_;
        }
        return MetaLocalBackend::Sync(keys) && !fail;
    }
...
private:
    ...
    void MaybeBlockLocationMapReadAfterRead() {
        std::unique_lock<std::mutex> lock(control_mutex_);
        if (!block_next_location_map_read_after_read_) {
            return;
        }
        block_next_location_map_read_after_read_ = false;
        location_map_read_after_read_entered_ = true;
        control_cv_.notify_all();
        control_cv_.wait(lock, [&] { return release_location_map_read_after_read_; });
    }
    ...
    bool block_next_location_map_read_after_read_ = false;
    bool location_map_read_after_read_entered_ = false;
    bool release_location_map_read_after_read_ = false;
...
TEST_F(CacheManagerTest, TestConcurrentReadFailureAndAddPreserveRefreshedUri) {
    ...
    // Pause after discovery captured the old location, before invalidation
    // enters its conditional RMW. The concurrent ADD must finish first.
    meta_backend->BlockNextLocationMapReadAfterRead();
    ...
    const bool read_blocked = meta_backend->WaitUntilLocationMapReadAfterReadEntered(std::chrono::seconds(1));
    if (!read_blocked) {
        meta_backend->ReleaseLocationMapReadAfterRead();
    }
    ...
    const auto add_status = add.wait_for(std::chrono::seconds(2));
    if (add_status != std::future_status::ready) {
        meta_backend->ReleaseLocationMapReadAfterRead();
    }
    ASSERT_EQ(std::future_status::ready, add_status);
    EXPECT_EQ(EC_OK, add.get().first);
    EXPECT_EQ(std::future_status::timeout, invalidation.wait_for(std::chrono::milliseconds(0)));
...
TEST_F(CacheManagerTest, TestReadFailureSurfacesPostDeleteSyncFailure) {
    const std::string exact_owner = "192.168.20.13:8080";
    const std::string mismatch_owner = "192.168.20.14:8080";
    const std::string observer = "192.168.20.15:8080";
    const int64_t key = 96'007;
    ASSERT_NE(nullptr, InstallEventReportBackend());
    auto *meta_backend = InstallControllableMetaBackend();
    ASSERT_NE(nullptr, meta_backend);

    ASSERT_EQ(EC_OK, CallReportEvent(MakeAddRequest(exact_owner, key, "exact"), "read_failure_sync_seed_exact").first);
    ASSERT_EQ(
        EC_OK,
        CallReportEvent(MakeAddRequest(mismatch_owner, key, "mismatch"), "read_failure_sync_seed_mismatch").first);
    const auto before = QueryRawEventReportUris(key);
    ASSERT_EQ(2u, before.size());
    const auto exact_uri = std::find_if(before.begin(), before.end(), [](const std::string &uri) {
        return uri.find("source=exact") != std::string::npos;
    });
    ASSERT_NE(before.end(), exact_uri);

    meta_backend->FailSyncCall(2);
    const auto [ec, response] =
        CallReportEvent(MakeReadFailureRequest(observer, key, {{"tp0", *exact_uri}}), "read_failure_sync_failure");
    EXPECT_EQ(EC_PARTIAL_OK, ec);
    ...
    const auto after = QueryRawEventReportUris(key);
    ASSERT_EQ(1u, after.size());
    EXPECT_NE(std::string::npos, after.front().find("source=mismatch"));
}
