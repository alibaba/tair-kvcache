...
TEST_F(WriteLocationManagerTest, FinishValidationMismatchDoesNotConsumeSession) {
    manager_.Put(
        "session", "instance-a", {1, 2, 3}, {"id1", "id2", "id3"}, {"tp0", "tp1"}, 1000, [](WriteLocationInfoPtr) {});
    WriteLocationManager::WriteLocationInfo info;

    EXPECT_EQ(WriteLocationManager::TakeResult::INSTANCE_ID_MISMATCH,
              manager_.GetAndDeleteForFinish("session", "instance-b", BlockMaskOffset{3}, 3, {"tp0"}, info));
    EXPECT_EQ(WriteLocationManager::TakeResult::BLOCK_MASK_MISMATCH,
              manager_.GetAndDeleteForFinish("session", "instance-a", BlockMaskVector{true, true}, 3, {"tp0"}, info));
    EXPECT_EQ(WriteLocationManager::TakeResult::BLOCK_MASK_MISMATCH,
              manager_.GetAndDeleteForFinish("session", "instance-a", BlockMaskOffset{4}, 3, {"tp0"}, info));
    EXPECT_EQ(WriteLocationManager::TakeResult::KEY_COUNT_MISMATCH,
              manager_.GetAndDeleteForFinish("session", "instance-a", BlockMaskOffset{3}, 2, {"tp0"}, info));
    EXPECT_EQ(WriteLocationManager::TakeResult::SPEC_NAME_MISMATCH,
              manager_.GetAndDeleteForFinish("session", "instance-a", BlockMaskOffset{3}, 3, {"tp-missing"}, info));
    EXPECT_EQ(1, manager_.ExpireSize());
    EXPECT_TRUE(manager_.HasLocationId("id1"));
