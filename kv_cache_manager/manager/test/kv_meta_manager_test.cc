...
public:
    explicit CapacityRejectingNfsBackend(std::shared_ptr<MetricsRegistry> metrics_registry)
        : NfsBackend(std::move(metrics_registry)) {}

    std::vector<std::pair<ErrorCode, DataStorageUri>>
    Create(const std::vector<std::string> &keys, size_t, const std::string &, std::function<void()> cb) override {
        create_attempts_.fetch_add(1, std::memory_order_acq_rel);
        if (cb) {
            cb();
        }
...
class PreAllocationRejectingNfsBackend : public NfsBackend {
public:
    explicit PreAllocationRejectingNfsBackend(std::shared_ptr<MetricsRegistry> metrics_registry)
        : NfsBackend(std::move(metrics_registry)) {}
...
class AmbiguousCreateFailureNfsBackend : public NfsBackend {
public:
    explicit AmbiguousCreateFailureNfsBackend(std::shared_ptr<MetricsRegistry> metrics_registry)
        : NfsBackend(std::move(metrics_registry)) {}
    ...
    Create(const std::vector<std::string> &keys, size_t, const std::string &, std::function<void()> cb) override {
        ...
        // Deliberately violate the KVMeta extension contract to verify that
        // the manager still fails closed when allocation outcome is unknown.
        return std::vector<std::pair<ErrorCode, DataStorageUri>>(keys.size(), {EC_IO_ERROR, DataStorageUri{}});
    }
};
...
public:
    ...
    std::vector<ErrorCode>
    Delete(const std::vector<DataStorageUri> &storage_uris, const std::string &, std::function<void()> cb) override {
        delete_attempts_.fetch_add(storage_uris.size(), std::memory_order_acq_rel);
        if (cb) {
            cb();
        }
        return std::vector<ErrorCode>(storage_uris.size(), delete_error_.load(std::memory_order_acquire));
    }
    ...
    bool IsKvMetaDeleteRetrySafe() const noexcept override { return false; }

    void SetDeleteError(ErrorCode ec) { delete_error_.store(ec, std::memory_order_release); }
...
private:
    std::atomic<ErrorCode> delete_error_{EC_OK};
...
protected:
    ...
    void UpdateStorageCandidates(const std::string &group_name, std::vector<std::string> storage_candidates) {
        const auto [group_ec, current_group] = registry_manager_->GetInstanceGroup(&request_context_, group_name);
        ASSERT_EQ(EC_OK, group_ec);
        ASSERT_TRUE(current_group);
        InstanceGroup updated(*current_group);
        updated.set_storage_candidates(std::move(storage_candidates));
        updated.set_version(current_group->version() + 1);
        ASSERT_EQ(EC_OK, registry_manager_->UpdateInstanceGroup(&request_context_, updated, current_group->version()));
    }
...
TEST_F(KvMetaManagerTest, KnownPreAllocationRejectionDoesNotCloseKvMeta) {
    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto original = storage_manager->GetDataStorageBackend("nfs_01");
    ASSERT_TRUE(original);

    auto rejecting = std::make_shared<PreAllocationRejectingNfsBackend>(metrics_registry_);
    ASSERT_EQ(EC_OK, rejecting->Open(original->GetStorageConfig(), request_context_.trace_id()));
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_["nfs_01"] = rejecting;
    }

    const auto [rejected_ec, rejected] =
        manager_->StartWrite(&request_context_, kInstanceId, {"pre-allocation-reject"}, {17}, 30);
    EXPECT_EQ(EC_NOENT, rejected_ec);
    EXPECT_TRUE(rejected.locations.empty());
    EXPECT_TRUE(rejected.write_session_id.empty());
    ...
    auto [retry_ec, retry] = manager_->StartWrite(&request_context_, kInstanceId, {"pre-allocation-reject"}, {17}, 30);
...
TEST_F(KvMetaManagerTest, AmbiguousCreateFailureIsNormalizedAndClosesKvMeta) {
    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto original = storage_manager->GetDataStorageBackend("nfs_01");
    ASSERT_TRUE(original);

    auto ambiguous = std::make_shared<AmbiguousCreateFailureNfsBackend>(metrics_registry_);
    ASSERT_EQ(EC_OK, ambiguous->Open(original->GetStorageConfig(), request_context_.trace_id()));
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_["nfs_01"] = ambiguous;
    }

    const auto [ec, result] =
        manager_->StartWrite(&request_context_, kInstanceId, {"ambiguous-create-failure"}, {17}, 30);
    EXPECT_EQ(EC_OUTCOME_UNKNOWN, ec);
    EXPECT_TRUE(result.locations.empty());
    EXPECT_TRUE(result.write_session_id.empty());
    EXPECT_TRUE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));

    const auto [get_ec, values] = manager_->Get(&request_context_, kInstanceId, {"ambiguous-create-failure"});
    ASSERT_EQ(EC_OK, get_ec);
    ASSERT_EQ(1, values.size());
    EXPECT_FALSE(values.front().found);
}
...
TEST_F(KvMetaManagerTest, ConditionalInsertRollbackNeverDeletesAnUnexpectedSameAllocationOwner) {
    ...
    EXPECT_TRUE(loser.locations.empty());
    EXPECT_TRUE(loser.write_session_id.empty());
    EXPECT_EQ(0, reused->DeleteAttempts());
    ...
    EXPECT_EQ(1, reused->DeleteAttempts());
    concurrent_manager->Shutdown();
...
TEST_F(KvMetaManagerTest, RecoveryFailsClosedAndRetainsItsLedgerAfterPhysicalDeleteException) {
    ...
    EXPECT_EQ(EC_OUTCOME_UNKNOWN, manager_->DoRecover());
    EXPECT_EQ(1, throwing->DeleteAttempts());
    EXPECT_TRUE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));
...
TEST_F(KvMetaManagerTest, RecoveryStopsAfterFirstUnknownDeleteAndRetainsEveryCleanupLedger) {
    ...
    EXPECT_EQ(EC_OUTCOME_UNKNOWN, manager_->DoRecover());
    // A provider failure is observed before crossing into the peer instance.
    // Continuing would turn one outage into one uncertain deletion per
    // instance before the caller can fence admission.
    EXPECT_EQ(1, failing->DeleteAttempts());
    EXPECT_TRUE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));
...
TEST_F(KvMetaManagerTest, ReclaimerYieldsAnActiveColdKeyWithinOneInstance) {
    constexpr const char *kGroup = "reclaim-active-cold-key-group";
    constexpr const char *kInstance = "reclaim-active-cold-key-instance";
    constexpr const char *kActiveKey = "a-active-object";
    constexpr const char *kCommittedKey = "z-committed-object";
    CreateReclaimGroup(kGroup, kInstance, 100, 0.8, 0);

    auto [start_ec, start] = manager_->StartWrite(&request_context_, kInstance, {kActiveKey}, {45}, 30);
    ASSERT_EQ(EC_OK, start_ec);
    std::this_thread::sleep_for(std::chrono::milliseconds(2));
    CommitObject(kInstance, kCommittedKey, 45);
    ...
    ASSERT_TRUE(WaitUntil([&]() { return indexer->GetStorageUsage() == 45; }, std::chrono::seconds(2)));
    EXPECT_NE(0, metrics_registry_->GetCounter("kv_meta_reclaimer.maintenance_touch_key_count").Get());
    auto [get_ec, values] = manager_->Get(&request_context_, kInstance, {kCommittedKey});
    ASSERT_EQ(EC_OK, get_ec);
    ASSERT_EQ(1, values.size());
    EXPECT_FALSE(values.front().found);

    LocationsPerKey active_locations;
    const auto exact = indexer->GetLocations(&request_context_,
                                             {KvMetaManager::InternalKey(kActiveKey)},
                                             {{KvMetaManager::StableLocationId(kActiveKey)}},
                                             active_locations);
    ASSERT_EQ(EC_OK, exact.ec);
    ASSERT_EQ(1, active_locations.size());
    ASSERT_EQ(1, active_locations.front().size());
    ASSERT_TRUE(active_locations.front().front());
    EXPECT_EQ(CLS_NEW, active_locations.front().front()->status());
...
TEST_F(KvMetaManagerTest, PaceReclaimerAttemptsAnAmbiguousReusableGaDeleteOnlyOnce) {
    constexpr const char *kGroup = "pace-at-most-once-reclaim-group";
    constexpr const char *kInstance = "pace-at-most-once-reclaim-instance";
    constexpr const char *kStorage = "pace_at_most_once_reclaim";
    constexpr const char *kKey = "pace-at-most-once-reclaim-object";
    constexpr std::uint64_t kSize = 90;

    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto pace = std::make_shared<DuplicateSingletonCreateTairMempoolBackend>(metrics_registry_);
    auto pace_spec = std::make_shared<TairMemPoolStorageSpec>();
    const StorageConfig pace_config(DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL, kStorage, pace_spec);
    ASSERT_EQ(EC_OK, pace->Open(pace_config, request_context_.trace_id()));
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_[kStorage] = pace;
    }
    CreateReclaimGroup(kGroup,
                       kInstance,
                       100,
                       ...
                       DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL,
                       kStorage);
    CommitObject(kInstance, kKey, kSize);
    pace->SetDeleteError(EC_IO_ERROR);

    auto indexer = cache_manager_->meta_indexer_manager()->GetMetaIndexer(KvMetaManager::InternalInstanceId(kInstance));
    ASSERT_TRUE(indexer);
    cache_manager_->cache_reclaimer()->SetSleepIntervalMs(&request_context_, 5);
    ASSERT_TRUE(manager_->ResumeMaintenance());
    ASSERT_TRUE(WaitUntil([&]() { return pace->DeleteAttempts() == 1; }, std::chrono::seconds(2)));
    ASSERT_TRUE(WaitUntil([&]() { return manager_->maintenance_cancelled_.load(std::memory_order_acquire); },
                          std::chrono::seconds(2)));

    // PACE's legacy GA can be reused and carries no generation token. A lost
    // response therefore stops only the KVMeta side path and retains the
    // durable tombstone; neither the worker nor recovery may replay the GA.
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_EQ(kSize, indexer->GetStorageUsage());
    CacheLocationMapVector maps;
    const auto metadata =
        indexer->GetLocationMapsForMaintenance(&request_context_, {KvMetaManager::InternalKey(kKey)}, maps);
    ...
    EXPECT_EQ(kSize, metrics_registry_->GetCounter("kv_meta_reclaimer.physical_delete_uncertain_bytes").Get());

    manager_->DoCleanup();
    pace->SetDeleteError(EC_OK);
    ASSERT_EQ(EC_OK, manager_->DoRecover());
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_EQ(0, indexer->GetStorageUsage());
    auto [get_ec, values] = manager_->Get(&request_context_, kInstance, {kKey});
...
TEST_F(KvMetaManagerTest, PaceRemoveBoundsAnUnknownDeleteToOneAttemptAndLogicalCleanup) {
    constexpr const char *kGroup = "pace-at-most-once-remove-group";
    constexpr const char *kInstance = "pace-at-most-once-remove-instance";
    constexpr const char *kStorage = "pace_at_most_once_remove";
    constexpr const char *kKey = "pace-at-most-once-remove-object";
    ...
    CreateReclaimGroup(kGroup,
                       ...
                       DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL,
                       kStorage);
    CommitObject(kInstance, kKey, 41);
    pace->SetDeleteError(EC_IO_ERROR);

    EXPECT_EQ(EC_OUTCOME_UNKNOWN, manager_->Remove(&request_context_, kInstance, {kKey}));
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_TRUE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));
    auto indexer = cache_manager_->meta_indexer_manager()->GetMetaIndexer(KvMetaManager::InternalInstanceId(kInstance));
    ASSERT_TRUE(indexer);
    EXPECT_EQ(0, indexer->GetStorageUsage());
    EXPECT_EQ(EC_SERVICE_NOT_LEADER, manager_->Remove(&request_context_, kInstance, {kKey}));
    EXPECT_EQ(1, pace->DeleteAttempts());
...
TEST_F(KvMetaManagerTest, RemoveIsolatesRetrySafeAndAtMostOnceDeleteFailures) {
    constexpr const char *kGroup = "mixed-policy-remove-group";
    constexpr const char *kInstance = "mixed-policy-remove-instance";
    constexpr const char *kPaceStorage = "pace_mixed_remove";
    constexpr const char *kNfsKey = "mixed-remove-nfs";
    constexpr const char *kPaceKey = "mixed-remove-pace";
    constexpr std::uint64_t kNfsSize = 37;
    constexpr std::uint64_t kPaceSize = 41;
    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto pace = std::make_shared<DuplicateSingletonCreateTairMempoolBackend>(metrics_registry_);
    const StorageConfig pace_config(
        DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL, kPaceStorage, std::make_shared<TairMemPoolStorageSpec>());
    ASSERT_EQ(EC_OK, pace->Open(pace_config, request_context_.trace_id()));
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_[kPaceStorage] = pace;
    }
    CreateReclaimGroup(kGroup,
                       kInstance,
                       1024,
                       ...
                       DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL,
                       kPaceStorage);
    CommitObject(kInstance, kPaceKey, kPaceSize);
    UpdateStorageCandidates(kGroup, {"nfs_01"});
    CommitObject(kInstance, kNfsKey, kNfsSize);

    auto original_nfs = storage_manager->GetDataStorageBackend("nfs_01");
    ASSERT_TRUE(original_nfs);
    auto nfs = std::make_shared<FaultingDeleteNfsBackend>(metrics_registry_, FaultingDeleteNfsBackend::Mode::kError);
    ASSERT_EQ(EC_OK, nfs->Open(original_nfs->GetStorageConfig(), request_context_.trace_id()));
    pace->SetDeleteError(EC_IO_ERROR);
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_["nfs_01"] = nfs;
    }

    EXPECT_EQ(EC_OUTCOME_UNKNOWN, manager_->Remove(&request_context_, kInstance, {kNfsKey, kPaceKey}));
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_EQ(1, nfs->DeleteAttempts());
    EXPECT_TRUE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));

    auto indexer = cache_manager_->meta_indexer_manager()->GetMetaIndexer(KvMetaManager::InternalInstanceId(kInstance));
    ASSERT_TRUE(indexer);
    // The ambiguous reusable GA is logically finalized and is never replayed.
    // The independent NFS failure keeps its path-unique tombstone and quota
    // charge so leader recovery can retry that object safely.
    EXPECT_EQ(kNfsSize, indexer->GetStorageUsage());
    auto [get_ec, values] = manager_->Get(&request_context_, kInstance, {kNfsKey, kPaceKey});
    ASSERT_EQ(EC_OK, get_ec);
    ASSERT_EQ(2, values.size());
    EXPECT_FALSE(values[0].found);
    EXPECT_FALSE(values[1].found);

    nfs->SetMode(FaultingDeleteNfsBackend::Mode::kSuccess);
    pace->SetDeleteError(EC_OK);
    manager_->DoCleanup();
    ASSERT_EQ(EC_OK, manager_->DoRecover());
    EXPECT_EQ(2, nfs->DeleteAttempts());
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_EQ(0, indexer->GetStorageUsage());
    ASSERT_TRUE(manager_->ResumeMaintenance());
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
        storage_manager->storage_map_["nfs_01"] = original_nfs;
    }
}

TEST_F(KvMetaManagerTest, ReclaimerKeepsMixedDeletePoliciesInIndependentBatches) {
    constexpr const char *kGroup = "mixed-policy-reclaim-group";
    constexpr const char *kInstance = "mixed-policy-reclaim-instance";
    constexpr const char *kPaceStorage = "pace_mixed_reclaim";
    constexpr const char *kNfsKey = "mixed-reclaim-nfs";
    constexpr const char *kPaceKey = "mixed-reclaim-pace";
    constexpr std::uint64_t kObjectSize = 45;
    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto pace = std::make_shared<DuplicateSingletonCreateTairMempoolBackend>(metrics_registry_);
    const StorageConfig pace_config(
    ...
    CreateReclaimGroup(kGroup,
                       ...
                       DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL,
                       kPaceStorage);
    CommitObject(kInstance, kPaceKey, kObjectSize);
    UpdateStorageCandidates(kGroup, {"nfs_01"});
    CommitObject(kInstance, kNfsKey, kObjectSize);
    ...
    ASSERT_TRUE(manager_->ResumeMaintenance());
    ASSERT_TRUE(WaitUntil([&]() { return pace->DeleteAttempts() == 1; }, std::chrono::seconds(2)));
    ASSERT_TRUE(nfs->WaitForDeleteAttempts(2, std::chrono::seconds(2)));

    // PACE succeeded and is finalized once. NFS failures remain retryable and
    // cannot be reclassified as an ambiguous reusable-address result merely
    // because both objects were selected by the same reclaim round.
    EXPECT_EQ(1, pace->DeleteAttempts());
    EXPECT_EQ(kObjectSize, indexer->GetStorageUsage());
    EXPECT_FALSE(manager_->maintenance_cancelled_.load(std::memory_order_acquire));
    EXPECT_GE(metrics_registry_->GetCounter("kv_meta_reclaimer.physical_delete_uncertain_object_count").Get(), 2);
...
TEST_F(KvMetaManagerTest, ReclaimerShrinksALargeBatchToThePendingByteBudget) {
    ...
    // KVMeta keeps the inherited logical selection size, but publishes bounded
    // physical batches so URL-based providers never receive an oversized
    // delete request. Sampling may require multiple reclaim rounds, therefore
    // 4096 objects need at least sixteen (and at most 4096) pending batches.
    const double pending_batches = metrics_registry_->GetGauge("kv_meta_reclaimer.pending_batch_count").Get();
    EXPECT_GE(pending_batches, 16);
    EXPECT_LE(pending_batches, 4096);
...
TEST_F(KvMetaManagerTest, BackendNoSpaceDoesNotTriggerKVCMReclamation) {
    constexpr const char *kGroup = "backend-capacity-isolation-group";
    constexpr const char *kInstance = "backend-capacity-isolation-instance";
    constexpr const char *kExisting = "existing-cache-object";
    CreateReclaimGroup(kGroup, kInstance, 1000, 1.0, 0);
    CommitObject(kInstance, kExisting, 70);

    auto storage_manager = registry_manager_->data_storage_manager();
    ASSERT_TRUE(storage_manager);
    auto original = storage_manager->GetDataStorageBackend("nfs_01");
    ASSERT_TRUE(original);
    auto capacity = std::make_shared<CapacityRejectingNfsBackend>(metrics_registry_);
    ...
    auto [ec, result] = manager_->StartWrite(&request_context_, kInstance, {"provider-rejected"}, {40}, 30);
    EXPECT_EQ(EC_NOSPC, ec);
    EXPECT_TRUE(result.locations.empty());
    EXPECT_EQ(1, capacity->CreateAttempts());

    // Provider allocation failures belong to the provider. KVCM must not
    // evict a healthy cache entry when its own quota and watermarks permit it.
    ...
    EXPECT_DOUBLE_EQ(0, metrics_registry_->GetGauge("kv_meta_reclaimer.admission_demand_group_count").Get());
    auto [get_ec, values] = manager_->Get(&request_context_, kInstance, {kExisting});
    ASSERT_EQ(EC_OK, get_ec);
    ASSERT_EQ(1, values.size());
    EXPECT_TRUE(values.front().found);
...
TEST_F(KvMetaManagerTest, ReclaimerIsolatesACorruptCandidateAndStillMakesProgress) {
    ...
    cache_manager_->cache_reclaimer()->SetSleepIntervalMs(&request_context_, 5);
    ASSERT_TRUE(manager_->ResumeMaintenance());
    // Do not poll through public Get here: every hit refreshes the valid
    // key's LRU timestamp and lets the test itself perturb which candidate is
    // coldest. Observe the authoritative usage/key counters, then verify the
    // public read result once reclamation has converged.
    ASSERT_TRUE(WaitUntil([&]() { return indexer->GetStorageUsage() == 30 && indexer->GetKeyCount() == 1; },
                          std::chrono::seconds(2)));
    cache_manager_->PauseReclaimer();
    ...
    auto [valid_ec, valid_values] = manager_->Get(&request_context_, kInstance, {kValidKey});
    ASSERT_EQ(EC_OK, valid_ec);
    ASSERT_EQ(1, valid_values.size());
    EXPECT_FALSE(valid_values.front().found);
