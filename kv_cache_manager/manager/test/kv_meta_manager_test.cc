...
protected:
    ...
    void UpdateStorageCandidates(const std::string &group_name, std::vector<std::string> storage_candidates) {
        ...
        ASSERT_EQ(EC_OK,
                  registry_manager_->UpdateInstanceGroup(&request_context_, updated, current_group->version()));
    }
...
TEST_F(KvMetaManagerTest, RemoveIsolatesRetrySafeAndAtMostOnceDeleteFailures) {
    ...
    ASSERT_TRUE(storage_manager);
    auto pace = std::make_shared<DuplicateSingletonCreateTairMempoolBackend>(metrics_registry_);
    const StorageConfig pace_config(DataStorageType::DATA_STORAGE_TYPE_TAIR_MEMPOOL,
                                    kPaceStorage,
                                    std::make_shared<TairMemPoolStorageSpec>());
    ASSERT_EQ(EC_OK, pace->Open(pace_config, request_context_.trace_id()));
    {
        std::unique_lock<std::shared_mutex> lock(storage_manager->rw_lock_);
...
TEST_F(KvMetaManagerTest, ForegroundRemoveSatisfiesBackendCapacityDemandWithoutExtraEviction) {
    constexpr const char *kGroup = "foreground-capacity-release-group";
    constexpr const char *kInstance = "foreground-capacity-release-instance";
    constexpr const char *kRemoved = "foreground-capacity-release-removed";
    constexpr const char *kRetained = "foreground-capacity-release-retained";
    CreateReclaimGroup(kGroup, kInstance, 1000, 1.0, 0);
    CommitObject(kInstance, kRemoved, 30);
    CommitObject(kInstance, kRetained, 30);
    ...
    ASSERT_TRUE(manager_->ResumeMaintenance());
    EXPECT_EQ(EC_NOSPC, manager_->StartWrite(&request_context_, kInstance, {"blocked"}, {20}, 30).first);
    EXPECT_DOUBLE_EQ(20, metrics_registry_->GetGauge("kv_meta_reclaimer.backend_capacity_demand_bytes").Get());

    ASSERT_EQ(EC_OK, manager_->Remove(&request_context_, kInstance, {kRemoved}));
    ...
    ASSERT_TRUE(WaitUntil(
        ...
        std::chrono::seconds(2)));
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    auto [get_ec, values] = manager_->Get(&request_context_, kInstance, {kRemoved, kRetained});
    ASSERT_EQ(EC_OK, get_ec);
    ASSERT_EQ(2, values.size());
    EXPECT_FALSE(values[0].found);
    EXPECT_TRUE(values[1].found);
...
TEST_F(KvMetaManagerTest, BackendCapacityRetriesDoNotDuplicateInFlightEviction) {
    ...
    // A retry while the first object is inside reader grace is covered by the
    // pending release. It must not publish a causally newer demand or retire
    // the second key after the first physical delete completes.
    EXPECT_EQ(EC_NOSPC, manager_->StartWrite(&request_context_, kInstance, {"backend-retry-new"}, {20}, 30).first);
    EXPECT_EQ(1, metrics_registry_->GetCounter("kv_meta_reclaimer.backend_capacity_demand_count").Get());
...
TEST_F(KvMetaManagerTest, ReclaimerIsolatesACorruptCandidateAndStillMakesProgress) {
    ...
    ASSERT_TRUE(WaitUntil(
        [&]() { return indexer->GetStorageUsage() == 30 && indexer->GetKeyCount() == 1; },
        std::chrono::seconds(2)));
    cache_manager_->PauseReclaimer();
