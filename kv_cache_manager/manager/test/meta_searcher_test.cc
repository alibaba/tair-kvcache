...
TEST_F(MetaSearcherTest, TestBatchGetMultiLocations) {
    ...
    // Add locations
    {
        ...
        std::vector<std::string> out_ids;
        ErrorCode ec =
            meta_searcher_->BatchAddLocation(request_context_.get(), {70000}, {v6d_loc_a}, out_ids);
        ASSERT_EQ(ec, ErrorCode::EC_OK);
    ...
    }
    {
        ...
        std::vector<std::string> out_ids;
        ErrorCode ec =
            meta_searcher_->BatchAddLocation(request_context_.get(), {70000}, {v6d_loc_b}, out_ids);
        ASSERT_EQ(ec, ErrorCode::EC_OK);
        std::vector<std::vector<MetaSearcher::LocationUpdateTask>> tasks = {{{out_ids[0], CLS_SERVING}}};
        std::vector<std::vector<ErrorCode>> results;
        meta_searcher_->BatchUpdateLocationStatus(request_context_.get(), {70000}, tasks, results);
    }
    {
        ...
        std::vector<std::string> out_ids;
        ErrorCode ec =
            meta_searcher_->BatchAddLocation(request_context_.get(), {70000}, {nfs_loc}, out_ids);
        ASSERT_EQ(ec, ErrorCode::EC_OK);
        std::vector<std::vector<MetaSearcher::LocationUpdateTask>> tasks = {{{out_ids[0], CLS_SERVING}}};
        std::vector<std::vector<ErrorCode>> results;
        meta_searcher_->BatchUpdateLocationStatus(request_context_.get(), {70000}, tasks, results);
    }
    {
        ...
        std::vector<std::string> out_ids;
        ErrorCode ec =
            meta_searcher_->BatchAddLocation(request_context_.get(), {70001}, {nfs_loc2}, out_ids);
        ASSERT_EQ(ec, ErrorCode::EC_OK);
        std::vector<std::vector<MetaSearcher::LocationUpdateTask>> tasks = {{{out_ids[0], CLS_SERVING}}};
        std::vector<std::vector<ErrorCode>> results;
    ...
    }
    {
        ...
        std::vector<std::string> out_ids;
        ErrorCode ec =
            meta_searcher_->BatchAddLocation(request_context_.get(), {70002}, {v6d_loc_c}, out_ids);
        ASSERT_EQ(ec, ErrorCode::EC_OK);
        std::vector<std::vector<MetaSearcher::LocationUpdateTask>> tasks = {{{out_ids[0], CLS_SERVING}}};
        std::vector<std::vector<ErrorCode>> results;
