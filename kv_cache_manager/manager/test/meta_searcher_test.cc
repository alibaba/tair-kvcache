...
TEST_F(HostCacheRemoteTest, PlainPrefixUsesSpecPresenceForTensorParallelReports) {
    Add(0, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L2, "worker:80", {"F0_N0", "F0_N1"});
    Add(1, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L1P5, "worker:80", {"F0"});
    Add(2, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L2, "worker:80", {"F0_N0"});
    Add(3, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L1P5, "other:80", {"F0"});
    Add(4, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L1P5, "worker:80", {"F0"});
    Add(5, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L2, "worker:80", {"F0_N1"});
    Store();
    for (size_t count : {size_t{0}, size_t{1}}) {
        for (bool p2p : {false, true}) {
            const auto matches = Query(count, p2p);
            ASSERT_EQ(1u, matches.size());
            EXPECT_EQ(std::make_pair(3L, 3L), matches.at("worker:80"));
        }
        EXPECT_EQ(std::make_pair(2L, 2L), Query(count, false, {}, true).at("worker:80"));
    }
    // Another engine's L1P5 does not fill a local gap or become a P2P source.
    // A physical V6D peer can fill it, even when it reports only one TP spec.
    Add(3, DataStorageType::DATA_STORAGE_TYPE_EVENT_REPORT_L2, "peer:80", {"F0_N1"});
    Store();
    EXPECT_EQ(std::make_pair(3L, 3L), Query(0, true).at("worker:80"));
    EXPECT_EQ(std::make_pair(3L, 3L), Query(1, false).at("worker:80"));
    EXPECT_EQ(std::make_pair(3L, 6L), Query(1, true).at("worker:80"));
}
