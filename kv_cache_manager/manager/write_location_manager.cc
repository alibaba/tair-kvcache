...
WriteLocationManager::TakeResult WriteLocationManager::SessionIdMap::GetAndDeleteForFinish(
...
WriteLocationManager::TakeResult WriteLocationManager::GetAndDeleteForFinish(
    const std::string &write_session_id,
    const std::string &instance_id,
    const BlockMask &success_block_mask,
    std::optional<size_t> expected_checksum_count,
    const std::vector<std::string> &location_spec_names,
    WriteLocationInfo &location_info) {
    return session_id_map_.GetAndDeleteForFinish(write_session_id,
                                                 instance_id,
                                                 success_block_mask,
