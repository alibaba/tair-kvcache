...
private:
        ...
        for (std::size_t instance_index = 0; instance_index < selected_instances.size(); ++instance_index) {
            ...
            for (std::size_t key_index = 0; key_index < keys.size(); ++key_index) {
                ...
                const bool relieves_key_pressure =
                    ...
                    (pressure.keys != 0 ||
                     (instance_pressure != pressure.keys_by_instance.end() && instance_pressure->second != 0));
    ...
    bool ReclaimGroup(RequestContext *request_context, const std::shared_ptr<const InstanceGroup> &group) {
        ...
        std::vector<RetiredItem> retired;
        {
            ...
            auto selected =
                SelectCandidates(std::move(candidate_keys),
                                 current_pressure,
                                 std::min({batch_size, pending_capacity.object_count, batch_slot_object_capacity}),
        ...
        if (!retired_items.empty()) {
            ...
            auto delete_and_finalize = [&](const DeletePartition &partition) -> bool {
                ...
                if (physical_ec != EC_OK) {
                    ...
                    KVCM_LOG_WARN("KVMeta will not retry an ambiguous reusable-address delete, item_count[%zu], ec[%d]",
...
KvMetaManager::StartWrite(RequestContext *request_context,
    ...
    if (!EncodeLeaseDeadline(write_start_time_us, write_timeout_seconds, persistent_write_deadline)) {
    ...
    for (const std::size_t request_index : missing_indices) {
        ...
        if (cancelled || expired) {
            if (!release_allocated_once(candidates,
                                        "KVMeta admission stop could not release all uncommitted allocations")) {
        ...
        try {
            ...
            release_allocated_once(candidates, "KVMeta could not release every allocation preceding a failed Create");
        ...
        if (create_result.size() != 1 ||
            ...
            const bool cleanup_complete =
                release_allocated_once(candidates, "KVMeta could not release every previously validated allocation");
            AddError(request_context,
        ...
        // still inside its advertised write window.
        ...
        if (location_ec != EC_OK || uri_size != value_sizes[request_index]) {
            ...
            const bool cleanup_complete =
                release_allocated_once(cleanup_items, "KVMeta could not release every invalid-size allocation");
            AddError(request_context,
...
ErrorCode KvMetaManager::DoRecover(std::function<bool()> should_abort) {
    ...
    const std::int64_t max_persisted_active_lease_seconds = limits_.max_write_timeout_seconds;
                    ...
                    if (!keys.empty()) {
                        ...
                        for (std::size_t i = 0; i < keys.size() && instance_recovery_ec == EC_OK; ++i) {
                            ...
                            for (const auto &[location_id, location] : locations[i]) {
                                ...
                                if (DecodeLeaseDeadline(location->create_time(), lease_deadline_us) &&
