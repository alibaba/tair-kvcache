...
void CacheManager::FillEmptyLocationSpecs(RequestContext *request_context,
    ...
    for (auto &location : locations) {
        if (location->spec_size() == 0) {
            auto mutable_loc = std::make_shared<CacheLocation>(*location);
            mutable_loc->set_spec_size(instance_info->location_spec_infos().size());
            for (auto &spec_info : instance_info->location_spec_infos()) {
                mutable_loc->push_location_spec(LocationSpec(spec_info.name(), ""));
            }
            location = mutable_loc;
        }
    }
}
...
ErrorCode CacheManager::ReportEvent(RequestContext *request_context,
    ...
    // Lazily inject cleanup callback (DoOpen runs before CacheManager exists).
    ...
    // Apply NODE_REGISTER first so subsequent events see a registered node.
    if (has_register) {
...
void CacheManager::CleanupHostLocations(const std::string &instance_id,
    ...
    // Skip if node re-registered (generation bumped) since cleanup was triggered.
    if (vineyard_backend) {
    ...
    // Abort scan if node re-registers mid-cleanup.
