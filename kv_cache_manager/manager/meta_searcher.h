...
public:
    ...
    // Upsert with caller-specified location_id and status. Same id replaces prior value.
    ...
    // Multi-location delete per key (vs BatchDeleteLocation which is one per key).
    ...
    // Scan all blocks and drop VINEYARD locations matching host_suffix. Long-running.
