...
ErrorCode MetaSearcher::BatchDeleteLocationSpecsIfMatch(
    ...
    auto *service_metrics_collector = dynamic_cast<ServiceMetricsCollector *>(request_context->metrics_collector());
    KVCM_METRICS_COLLECTOR_CHRONO_MARK_BEGIN(service_metrics_collector, MetaSearcherIndexerReadModifyWriteLocation);
    // Drain accepted persistent writes and validate both metadata layers under
    // the shard lock. A stale hot value must not authorize deleting a newer
    // persistent URI after a partial mirror failure.
    auto result = meta_indexer_->ReadModifyWriteLocationsForMaintenance(request_context,
                                                                        flat_keys,
                                                                        flat_location_ids,
                                                                        modifier,
                                                                        /*adjust_reclaimed_key_count=*/true,
                                                                        /*require_consistent_layers=*/true);
    KVCM_METRICS_COLLECTOR_CHRONO_MARK_END(service_metrics_collector, MetaSearcherIndexerReadModifyWriteLocation);

    bool malformed_result = result.per_location_error_codes.size() != flat_to_original_task_indices.size() ||
                            result.post_write_error_codes.size() != flat_to_original_task_indices.size();
    for (size_t flat_index = 0; flat_index < flat_to_original_task_indices.size(); ++flat_index) {
        ...
        if (flat_index >= result.per_location_error_codes.size() ||
            flat_index >= result.post_write_error_codes.size() ||
            result.per_location_error_codes[flat_index].size() != 1) {
            task_result.ec = EC_MISMATCH;
            malformed_result = true;
            continue;
        }
        const ErrorCode mutation_ec = result.per_location_error_codes[flat_index].front();
        if (mutation_ec == EC_OK &&
        ...
        task_result.ec = mutation_ec != EC_OK ? mutation_ec : result.post_write_error_codes[flat_index];
    }
