...
CacheLocationConstPtr PriorityThenRandomSLPolicy::SelectForMatch(CacheLocationMap &location_map,
                                                                 CheckLocDataExistFunc check_loc_data_exist,
                                                                 std::vector<std::string> &out_prune_loc_ids) const {
    thread_local std::mt19937 rng(std::random_device{}());

    std::vector<CacheLocationConstPtr> candidates;
    ...
    std::vector<CacheLocationConstPtr> top_tier;
