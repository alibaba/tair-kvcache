...
std::vector<ErrorCode> MetaRedisBackend::GetLocationIds(RequestContext * /*request_context*/,
    ...
    std::vector<ErrorCode> results =
        handle->GetFieldNamesWithPrefix(full_keys, LOCATION_PREFIX, raw_field_names_vec);
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        for (const auto &field_name : raw_field_names_vec[i]) {
            if (field_name.size() > LOCATION_PREFIX.size() &&
                field_name.rfind(LOCATION_PREFIX, 0) == 0) {
