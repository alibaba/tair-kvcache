...
TEST_F(CacheLocationTest, DefaultBlockHashIsZero) {
    CacheLocation loc;
    EXPECT_EQ(loc.block_hash(), 0);
}

TEST_F(CacheLocationTest, BlockHashRoundTripsThroughJson) {
    CacheLocation loc;
    loc.set_id("loc-1");
    ...
    loc.set_block_hash(0x0123456789ABCDEFLL);

    const std::string json = loc.ToJsonString();
    EXPECT_NE(json.find("block_hash"), std::string::npos);

    CacheLocation parsed;
    ASSERT_TRUE(parsed.FromJsonString(json));
    EXPECT_EQ(parsed.block_hash(), 0x0123456789ABCDEFLL);
    EXPECT_EQ(parsed.id(), "loc-1");
}

TEST_F(CacheLocationTest, LegacyJsonWithoutBlockHashDefaultsToZero) {
    // 老 meta 存量数据：不带 block_hash 字段。反序列化后 block_hash 必须为 0，
    ...
    EXPECT_EQ(parsed.block_hash(), 0);
    EXPECT_EQ(parsed.id(), "loc-1");
    EXPECT_EQ(parsed.status(), CacheLocationStatus::CLS_SERVING);
}

TEST_F(CacheLocationTest, NegativeBlockHashIsPreserved) {
    ...
    loc.set_type(DataStorageType::DATA_STORAGE_TYPE_NFS);
    loc.set_spec_size(0);
    loc.set_block_hash(-1);
    const std::string json = loc.ToJsonString();
    CacheLocation parsed;
    ASSERT_TRUE(parsed.FromJsonString(json));
    EXPECT_EQ(parsed.block_hash(), -1);
}
