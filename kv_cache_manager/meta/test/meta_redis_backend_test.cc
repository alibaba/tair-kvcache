...
TEST_F(MetaRedisBackendTest, TestInvalidClientPoolSize) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillRepeatedly(Invoke([]() {
        ...
        EXPECT_CALL(mock_redis_client->Executor(), Open()).WillRepeatedly(Return(true));
...
TEST_F(MetaRedisBackendTest, TestOpenAndClose) {
...

    {
        ...
        EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
            StandardUri empty_storage_uri;
            auto mock_redis_client = std::make_unique<MockRedisClient>(empty_storage_uri);
            EXPECT_CALL(mock_redis_client->Executor(), Open()).WillOnce(Return(false));
            return mock_redis_client;
        }));
        ASSERT_EQ(EC_OK, meta_redis_backend_->Init("instance_0", meta_storage_backend_config_));
...
TEST_F(MetaRedisBackendTest, TestSimple) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        ...
        put_replies.emplace_back(MakeFakeReplyInteger(1));
        EXPECT_CALL(
            mock_redis_client->Executor(),
            ExecuteBatch(ElementsAre(
        ...
        EXPECT_CALL(
            mock_redis_client->Executor(),
            ExecuteBatch(ElementsAre(ElementsAre(StrEq("EXISTS"), StrEq("kvcache:instance_instance_0:cache_1")),
        ...
        EXPECT_CALL(
            mock_redis_client->Executor(),
            ExecuteBatch(ElementsAre(ElementsAre(StrEq("HGETALL"), StrEq("kvcache:instance_instance_0:cache_1")),
        ...
        EXPECT_CALL(mock_redis_client->Executor(),
                    ExecuteBatch(ElementsAre(ElementsAre(StrEq("SCAN"),
                                                         StrEq("0"),
                                                         StrEq("MATCH"),
        ...
        EXPECT_CALL(mock_redis_client->Executor(), ExecuteBatch(ElementsAreArray(randomkey_commands)))
...
TEST_F(MetaRedisBackendTest, TestGroupLruMissingRedisTimestampRetainsCandidates) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        StandardUri empty_storage_uri;
        auto mock_redis_client = std::make_unique<MockRedisClient>(empty_storage_uri);
        EXPECT_CALL(*mock_redis_client, IsContextOk()).WillRepeatedly(Return(true));
        ...
        std::vector<ReplyUPtr> property_replies;
        property_replies.emplace_back(MakeFakeReplyArrayString({std::nullopt}));
        property_replies.emplace_back(MakeFakeReplyArrayString({""}));
        EXPECT_CALL(
            *mock_redis_client,
            TryExecPipeline(ElementsAre(
                ElementsAre(StrEq("HMGET"), StrEq("kvcache:instance_instance_0:cache_1"), StrEq(PROPERTY_LRU_TIME)),
    ...
    ASSERT_EQ(EC_OK, meta_redis_backend_->Open());
    ReclaimCandidateVector candidates;
    ASSERT_EQ(EC_OK, meta_redis_backend_->SampleReclaimCandidates(nullptr, 2, candidates, true));
    ASSERT_EQ(2, candidates.size());
    EXPECT_EQ(1, candidates[0].key);
    EXPECT_EQ(0, candidates[0].last_access_time_us);
    EXPECT_EQ(2, candidates[1].key);
...
TEST_F(MetaRedisBackendTest, TestRedisError) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        StandardUri empty_storage_uri;
        auto mock_redis_client = std::make_unique<MockRedisClient>(empty_storage_uri);
        EXPECT_CALL(mock_redis_client->Executor(), Open()).WillOnce(Return(true)).WillRepeatedly(Return(false));
        return mock_redis_client;
    }));
...
TEST_F(MetaRedisBackendTest, TestExistsFieldWithPrefix) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        ...
        EXPECT_CALL(mock_redis_client->Executor(),
                    ExecuteBatch(ElementsAre(ElementsAre(StrEq("HSCAN"),
...
TEST_F(MetaRedisBackendTest, TestExistsFieldWithPrefixIgnoresTombstone) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        ...
        exists_replies.emplace_back(MakeFakeReplyInteger(1));
        exists_replies.emplace_back(MakeFakeReplyInteger(1));
        EXPECT_CALL(mock_redis_client->Executor(), ExecuteBatch(testing::_))
            .WillOnce(Return(ByMove(std::move(exists_replies))));
...
TEST_F(MetaRedisBackendTest, TestDeleteFieldsEmptyFieldList) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        ...
        // No HDEL should be issued — ExecuteBatch must not be called.
        EXPECT_CALL(mock_redis_client->Executor(), ExecuteBatch(testing::_)).Times(0);
        return mock_redis_client;
    }));
...
TEST_F(MetaRedisBackendTest, TestGet) {
    EXPECT_CALL(*meta_redis_backend_, CreateRedisClient()).WillOnce(Invoke([]() {
        ...
        EXPECT_CALL(mock_redis_client->Executor(),
                    ExecuteBatch(ElementsAre(ElementsAre(
