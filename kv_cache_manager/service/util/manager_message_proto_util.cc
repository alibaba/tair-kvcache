...
ChecksumAlgo FromAdminProtoChecksumAlgo(proto::admin::ChecksumAlgo algo) {
    ...
    default:
        // Preserve "unknown" as an invalid internal value instead of silently
        // degrading it to CA_UNSPECIFIED. StorageConfig::ValidateRequiredFields
        // can then reject numeric enum values sent by a newer or malformed
        // client even when checksum computation is currently disabled.
        return static_cast<ChecksumAlgo>(std::numeric_limits<std::underlying_type_t<ChecksumAlgo>>::max());
    }
}
...
void DataIntegrityConfigFromAdminProto(const proto::admin::DataIntegrityConfig &src, DataIntegrityConfig &dst) {
    ...
    dst.set_enable_inline_header(src.enable_inline_header());
    dst.set_inline_header_version(src.inline_header_version());
    // Proto3 cannot distinguish an omitted enum from CA_UNSPECIFIED. Preserve
    // the C++ default for a disabled config, while an enabled config with no
    // algorithm remains invalid and is rejected by StorageConfig::Validate.
    if (src.algo() != proto::admin::CA_UNSPECIFIED || src.enable_meta_checksum()) {
...
void ProtoConvert::StorageConfigToProto(const StorageConfig &storage_config,
    ...
    if (!storage_config.integrity().IsDefault()) {
...
void ProtoConvert::StorageFromProto(const proto::admin::StorageConfig *proto_storage_config,
    ...
    storage_config.set_integrity(DataIntegrityConfig{});
