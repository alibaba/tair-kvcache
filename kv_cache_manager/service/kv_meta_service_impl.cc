...
bool HasBoundedKvMetaKeys(const RepeatedStrings &keys, const KvMetaManager::Limits &limits) {
    if (keys.empty() || static_cast<std::size_t>(keys.size()) > limits.max_batch_items) {
        return false;
    }
    return std::all_of(keys.begin(), keys.end(), [&](const std::string &key) {
        return !key.empty() && key.size() <= limits.max_key_bytes;
    });
}
...
void KvMetaServiceImpl::Get(RequestContext *request_context,
    ...
    if (!HasBoundedKvMetaKeys(request->keys(), kv_meta_manager_->limits())) {
        SetDirectError(request_context,
                       status,
                       proto::kv_meta::INVALID_ARGUMENT,
                       "KVMeta Get keys exceed the configured request limits");
        return;
    }
...
void KvMetaServiceImpl::PutStart(RequestContext *request_context,
    ...
    if (!HasBoundedKvMetaKeys(request->keys(), kv_meta_manager_->limits()) ||
        request->value_sizes_size() != request->keys_size()) {
        SetDirectError(request_context,
                       status,
                       proto::kv_meta::INVALID_ARGUMENT,
                       "KVMeta PutStart keys or value_sizes exceed the configured request limits");
        return;
    }
    const std::vector<std::string> keys(request->keys().begin(), request->keys().end());
...
void KvMetaServiceImpl::Remove(RequestContext *request_context,
    ...
    if (!HasBoundedKvMetaKeys(request->keys(), kv_meta_manager_->limits())) {
        SetDirectError(request_context,
                       status,
                       proto::kv_meta::INVALID_ARGUMENT,
                       "KVMeta Remove keys exceed the configured request limits");
        return;
    }
    const std::vector<std::string> keys(request->keys().begin(), request->keys().end());
