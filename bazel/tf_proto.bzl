...
def cc_proto_library(
    ...
    if use_grpc_plugin:
        cc_libs += ["@grpc//:grpc++"]
        cc_libs_grpc_header_only += ["@grpc//:grpc++_public_hdrs"]
    if default_header:
        header_only_name = name
        impl_name = name + "_impl"
        grpc_header_only_impl_name = impl_name + "_grpc_header_only"
    else:
    ...
    native.cc_library(
        name = grpc_header_only_impl_name,
        srcs = gen_srcs,
        hdrs = gen_hdrs,
        deps = cc_libs_grpc_header_only + deps,
        includes = includes,
        **kargs
    )
    native.cc_library(
