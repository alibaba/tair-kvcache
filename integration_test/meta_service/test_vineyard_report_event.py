...
V8 batch protocol semantics (vs V7):
  ...
  - ReportEventResponse uses CommonResponseHeader (header.status.code /
    header.status.message) for the overall RPC status, and may carry per-
    EventItem results in `item_results` when partial failures happen.
