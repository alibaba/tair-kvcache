...
同一个 key 已 committed 时，必须与本次 `value_sizes` 完全一致才返回 `key_mask=true`；尺寸不同直接返回
`SIZE_MISMATCH`。同一个 key 仍是 active 写入时整批返回 `WRITE_IN_PROGRESS`，避免另一个对象客户端在原写者
最终回滚时误报保存成功。上述判断都发生在新 allocation 之前。跨进程并发 PutStart 的条件插入失败后，服务端
会重新读取赢家 metadata，再次校验 exact key、状态、storage URI/backend 和尺寸；只有 committed 且尺寸相同
才能当作命中，active 赢家会触发本次候选 allocation 的安全回滚并返回 `WRITE_IN_PROGRESS`。
...
观察下具备线性化的同一瞬间可见性。`PutFinish` 和 session timeout 的 finalization 也持有对应 KVMeta group 的
admission shard，直到 metadata 持久化和物理删除全部结束；因此回滚刚暴露出的 miss 不会与旧 allocation 的
删除并发进入下一代 `PutStart`。
...
请求不会使 writer 仍持有的 URI 提前失效。对 committed 对象，`Remove` 从 metadata 条件删除、持久化到
物理释放结束始终持有 KVMeta group 的 admission shard；同 key 的下一代 `PutStart` 只能在旧物理对象删除
完成后进入分配，避免可复用地址型 backend 上发生旧删除误伤新 generation 的 ABA 竞态。

`Trim` 还把 session 从可查询表取出之后的提交/回滚阶段计为 in-flight；只有 metadata 和物理存储操作全部
结束才解除保护，避免 Trim 与回滚同时删除同一 allocation。该保护只使用 KVMeta 自己的 group admission lock
和 session 状态，不进入固定 block KV-cache 的写入或回收链路。

通用对象在 metadata 中保持 `CLS_NEW`，避免进入按定长 block 设计的 reclaimer/migration。正
`create_time` 是 active marker；新版写入会在其中带 tag 持久化本次写租约的 wall-clock 截止时间，负值是
KVMeta 私有 committed marker。Get 只返回 committed 对象。新 leader 对未到期的 tagged active 对象保持服务门
关闭，截止后才清理；等待可被降主/Stop 以至多 100ms 的粒度取消，并由 `max_write_timeout_seconds` 限定上界，
避免旧 leader 已返回 `PutStart`、客户端仍在合法写数据时提前删除 allocation。滚动升级时，旧版本留下的无
tag 正 marker 按“创建时间 + `max_write_timeout_seconds`”保守推导截止时间；它没有记录请求的实际 timeout，
因此只能使用服务端上限。恢复删除使用完整序列化值做条件保护；无法确认归属时宁可保留数据，不会冒险删除
可能已被新一代 metadata 引用的 URI。跨节点 lease 判断依赖 wall clock，HA 部署必须保持 KVCM 节点时钟同步；
如果时钟漂移可能超过写租约，运维应把该漂移计入 `max_write_timeout_seconds`，否则新 leader 可能提前判断到期。
...
写租约从服务端收到 `PutStart` 后开始计时，allocation、metadata reservation、数据面写入和 `PutFinish` 共用
这一窗口；若服务端在登记 session 前已经耗尽窗口，会回滚 allocation 并返回超时。metadata reservation、
commit 和 delete 均经过 `Sync` 持久化屏障；删除只有在 metadata delete 已持久化后才
...
URI 和 PutFinish，任一步失败都会以失败 mask 回滚已开始的 session。对 `key_mask=true` 的项还会用 `Get`
确认其已 committed，兼容滚动升级中仍把 active 对象 mask 掉的旧服务端；RTP 使用随机对象 key 的全 miss
写入不增加这次确认 RPC。`LoadObjects` 在任何数据 IO 前验证全部 key 命中、value size、URI 和 buffer。
每个变长对象以 singleton SDK 调用搬运，但同一批次仍共享 wrapper
