...
GC 将 `MightExist` 已确认缺失的 URI 传给 Executor，由对应 Backend 的 `ShouldSkipConfirmedMissingBackendDelete()` 决定是否跳过后端删除。TairMempool（`pace` / `pace_ssd`）默认仍发送 Backend Delete，以便经 Free 清理共享 KVCS 残留；仅当对应 Storage 的 `skip_confirmed_missing_backend_delete=true` 时跳过这些 URI。其他后端保留跳过行为，`metadata_only` 始终只删元数据。Location 的条件删除、并发保护和用量扣减不变。`Delete` 返回 `EC_NOENT` 也视为幂等成功。真实存储删除失败由 Executor 按 URI 记录 WARN，包含 instance、storage、URI 和错误码。`PlanExecuteResult.error_logged` 仅在结果中的所有失败已有诊断时置为 true，GC 始终记录结果指标，但只对尚未记录的错误输出 action WARN；准入失败、worker 异常和 EventReport metadata 清理异常因此仍可见。扫描错误日志另带 cursor 和 error stage。
