...
| **event** | `event/` | 轻量事件总线。`EventManager` 将领域事件分发给注册的 `EventPublisher`；除默认日志发布外，可将 cache read 转成 `TraceQueryRequest` 并投递到 Optimizer gRPC 订阅队列。 |
| **protocol** | `protocol/protobuf/` | gRPC/proto 契约。定义 meta/admin/debug/kv_meta 和 optimizer event stream 服务，生成 C++ 与 Python 桩。 |
