...
跨轮顺序不仅考虑“是否开始采样”，还要考虑“是否获得 Location 检查机会”。A、B 都已采样，但 A 的 Location 读取耗尽本轮时间时，B 下轮优先检查；不能只把 B 的采样排到前面，否则采样更快的 A 仍可能抢先检查。上一轮收集不完整时，队首等待项保留首次 Location 检查机会，其他项仍可并行采样，但要等它完成采样并尝试检查，或采样失败后，才能开始自己的 Location 检查。其余检查不必等待其他慢采样项。

已尝试 Location 检查的项无论成功、失败还是超时，都让出顺序；采样明确失败的项也让出顺序。优先等待项若连采样也未能在 deadline 内完成，本轮结束后同样让出顺序，避免它长期阻塞后续项。未获检查机会的项保留相对顺序排在前面；正常完整收集后取消这项优先等待约束。线程池已满且本轮没有发起任何采样时，不消耗等待项的优先权。

这是同步读取下的轻量修复：慢 Location I/O 仍可能耗尽某一轮时间，优先项采样超时也可能使当轮无进展；轮转解决的是其他 Instance 长期没有检查机会的问题，不承诺同一轮隔离所有慢 I/O。没有新增线程池，也没有把 pending、迁移保护等资格判断移到 worker。
...
- 用独立的 Group ID 队列记录采样与 Location 检查的等待顺序，并记录下一轮是否优先检查队首项。每轮移除不再有效的 Instance，新增项排在已有等待项之后；不保留上轮候选，不能复用上轮未完成的半份结果。
