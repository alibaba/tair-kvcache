...
- vLLM Connector 集成测试： ```bazelisk test //integration_test/vllm_connector:connector_lifecycle_test --test_output=all```
  - 详细说明见 [vllm_connector_integration_test.md](vllm_connector_integration_test.md)
- 依赖Redis的测试：
...
A: 增加超时时间：
```bash
bazel test //integration_test/xxxx:xxx --test_timeout=300
```
...
各 Connector 集成测试的特定调试方式请参见对应文档，例如 [vLLM Connector 集成测试](vllm_connector_integration_test.md)。
