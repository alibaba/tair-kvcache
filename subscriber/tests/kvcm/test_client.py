    ...
    assert events == [
        ...
        },
        {
            ...
            "block_delete": {
                "block_key": "13",
                "medium": "mem",
                "specs": [{"name": "vllm_1", "uri": "vllm://10.0.0.8:9000/mem"}],
            },
        },
    ...
    assert _client()._report_events_for_batches([batch]) == [
        {
            ...
            "block_delete": {
                "block_key": "13",
                "medium": "",
                "specs": [{"name": "vllm_1", "uri": "vllm://10.0.0.8:9000/"}],
            },
        }
    ]
...
def test_register_and_report_use_per_group_specs() -> None:
    metadata = [
        KvCacheGroupSpec(group_idx=0, kind="full_attention", block_size=16),
        KvCacheGroupSpec(group_idx=1, kind="mamba", block_size=16, sliding_window=4096),
        KvCacheGroupSpec(group_idx=2, kind="sliding_window", block_size=16),
    ]
    client = KvcmClient(
        SubscriberConfig(),
        medium_mapper=_vllm_medium_mapper,
    ...
    assert request["location_spec_infos"] == [
        {"name": "full_0", "size": 16},
        {"name": "linear_1", "size": 16},
        {"name": "sliding_window_2", "size": 16},
    ]
    assert request["location_spec_groups"] == [
        {"name": "full_0", "spec_names": ["full_0"]},
        {"name": "linear_1", "spec_names": ["linear_1"]},
        {"name": "sliding_window_2", "spec_names": ["sliding_window_2"]},
    ]
    ...
    assert events[0]["block_add"]["specs"] == [
        {"name": "full_0", "uri": "vllm://10.0.0.8:9000/hbm"}
    ]
    assert events[1]["block_delete"]["specs"] == [
        {"name": "linear_1", "uri": "vllm://10.0.0.8:9000/hbm"}
    ]
...
async def test_learn_mode_warns_after_registering_default_specs(mocker) -> None:
    fake_sdk = FakeSdkClient()
    ...
    await client.start()
    client.update_group_metadata(
        {0: KvCacheGroupSpec(group_idx=0, kind="full_attention", block_size=16)}
    )

    warning.assert_any_call(
        "learn-mode registered KVCM with default specs; later learned group metadata "
        "cannot update the active registration",
        step="kv_metadata_learn",
        tags={"registered_spec_names": ["vllm_1"], "block_size": 1},
    )
...
async def test_start_without_endpoint_drops_batches_then_registers_after_recovery(
    ...
    await queue.put(mocker.Mock(batches=batches, epoch_snapshot=1, on_delivery=None))
    ...
    # Let the retry loop spin a few times while client is not ready.
    await asyncio.sleep(0.01)

    # Recover: client becomes ready and registers via heartbeat.
    fake_sdk.ready = True
    await asyncio.wait_for(registered.wait(), timeout=0.2)

    # The retry loop should now deliver the batch successfully.
    await asyncio.wait_for(queue.join(), timeout=1.0)

    fake_sdk.report_event.assert_awaited()
    sender.cancel()
